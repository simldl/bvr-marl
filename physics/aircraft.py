...
from physics.physics import AIRSPEED_OF_SOUND, PhysicsParams, get_speed_of_sound
...
from physics.afterburner import Afterburner, AfterburnerParams
...
class AircraftPhysics(FlyingPhysics):
    ...
    class Params(PhysicsParams):
        ...
        engine_thrust_profile: dict = field(default_factory=lambda: {"full_thrust_speed": 154.3, "optimal_speed": 257.2})
    ...
    def get_base_drag_cd(self, v_mps: float, alt_m: float) -> float:
        ...
        if mach < self.mach_t1:
            ...
            base = self.cd0_hi
        # Add AB plume/nozzle drag using the afterburner’s spool-weighted ΔCD0
    ...
    def get_cl_max(self, v_mps: float, alt_m: float) -> float:
        ...
        # Anchors (clean fighter, consistent with your table)
        CL_sub, CL_trans, CL_sup = 1.7, 1.3, 1.1

        def s(x, x0, w):  # logistic blend
            return 1.0 / (1.0 + math.exp(-(x - x0)/w))

        # Smooth transitions around 0.85 and 1.20
        w1 = 1.0 - s(M, 0.85, 0.04)                        # subsonic
        w2 = s(M, 0.85, 0.04) * (1.0 - s(M, 1.20, 0.06))   # transonic
        w3 = s(M, 1.20, 0.06)                               # supersonic
    ...
    def compute_instantaneous_load_factor(self, v_mps: float, alt_m: float) -> float:
        ...
        # Aerodynamic load factor limit
        n_aero = (q * self.A_m2 * cl_max) / (self.mass_kg * self.g)
        
        # Physical load factor is minimum of aerodynamic and structural limits
        n_max_phys = min(n_aero, self.n_max)
        
        return max(1.0, n_max_phys)  # Cannot be less than 1g
    
    def compute_sustained_load_factor(self, v_mps: float, alt_m: float, throttle: float = 1.0) -> float:
        """
        ...
        # Thrust/drag-limited load factor
        numerator = (T_avail - q * self.A_m2 * CD0) * q * self.A_m2
        denominator = k * W**2
        
        if numerator <= 0 or denominator <= 0:
            n_prop = 1.0  # Cannot sustain any load factor
        else:
            n_prop = math.sqrt(numerator / denominator)
        
        # Sustained load factor is minimum of physical and propulsion limits
        n_sust = min(n_max_phys, max(1.0, n_prop))
    ...
    def compute_sustained_turn_rate(self, v_mps: float, alt_m: float, throttle: float = 1.0) -> float:
        """
