...
from simulator.utils.angles import signed_yaw_deg_diff, normalize_angle, yaw_geo_to_math, yaw_math_to_geo
from simulator.utils.geodesics import geodetic_direct
from simulator.core.helpers import Position
from physics.physics import ForceModel
from physics.physics import AIRSPEED_OF_SOUND, get_speed_of_sound
from physics.physics import BasePhysics, PhysicsParams
...
class FlyingPhysics(BasePhysics):
    def __init__(self, params: PhysicsParams):
        ...
        # Aircraft enable energy-aware protections by default; missiles turn this off.
        self.enable_energy_protections = True

        # Use parameters from PhysicsParams (keeps defaults consistent)
        ...
        # Internal time accumulator for helper timing (keeps API unchanged)
    ...
    def get_pitch_limit_deg(self, pos, speed_mps) -> float:
        # Base implementation without energy protections (used by missiles)
        return 90.0  # Allow full pitch authority
    ...
    def update_yaw_deg(self, pos: Position, yaw_deg, desired_yaw_deg, speed_mps, dt, throttle=1.0, use_sustained=False) -> Tuple[float, float]:
        """
        ...
        if hasattr(self, 'compute_instantaneous_turn_rate') and hasattr(self, 'compute_sustained_turn_rate'):
            if use_sustained:
            ...
            # Fallback to original method for other physics classes
            g = self.g
            S = self.A_m2
            m = self.mass_kg
            W = m * g
    ...
    def update_pitch_deg(self, pitch_deg, desired_pitch_deg, speed_mps, dt, pos=None):
        ...
        limited_desired_pitch = max(min(desired_pitch_deg, pitch_limit), -pitch_limit)
    ...
    def update_roll(
        self, speed_mps: float, omega_rad: float, pitch_deg: float) -> float:
        pitch_rad = math.radians(pitch_deg)
        return math.degrees(
            math.atan((speed_mps * omega_rad * math.cos(pitch_rad)) / self.g)
        )
    ...
    def is_in_stall_condition(self, speed_mps: float, alt_m: float, load_factor: float = 1.0) -> bool:
        """
    ...
    def should_enter_stall_recovery(self, speed_mps: float, alt_m: float, load_factor: float = 1.0, 
    ...
    def should_exit_stall_recovery(self, speed_mps: float, alt_m: float, current_time: float = 0.0) -> bool:
    ...
    def compute_stall_recovery_controls(self, current_pitch_deg: float, current_throttle: float) -> tuple[float, float]:
        """
    ...
    def _update_orientation(self, pos: Position, pitch_deg: float, desired_pitch_deg: float,
                            yaw_deg: float, desired_yaw_deg: float,
                            speed_mps: float, dt: float) -> Tuple[float, float, float]:
        pitch_limit = self._service_ceiling_pitch_limit(pos.alt)
        pitch_deg = self.update_pitch_deg(pitch_deg, min(desired_pitch_deg, pitch_limit), speed_mps, dt)
        yaw_deg, omega_rad = self.update_yaw_deg(pos, yaw_deg, desired_yaw_deg, speed_mps, dt)
        return pitch_deg, yaw_deg, omega_rad

    def _compute_motion_vectors(self, pitch_rad: float, yaw_deg_geo: float, speed_mps: float, dt: float):
        """
    ...
    def _integrate_energy_and_speed(self, pos: Position, alt2: float, speed_mps: float,
                                    pitch_rad: float, n: float, throttle: float, dt: float) -> float:
    ...
    def compute_movement(self, pos: Position,
                         yaw_deg: float, desired_hdg_deg: float,
                         pitch_deg: float, desired_pitch_deg: float,
                         speed_mps: float, throttle: float,
                         dt: float) -> Tuple[float, float, float, float, float, float, float]:
        ...
        pitch_deg, yaw_deg, omega_rad = self._update_orientation(
            pos, pitch_deg, desired_pitch_deg,
            yaw_deg, desired_hdg_deg,
            speed_mps, dt
        )
        ...
        ground_track, ground_dist, dz = self._compute_motion_vectors(pitch_rad, yaw_deg, speed_mps, dt)
        lat2, lon2, alt2 = geodetic_direct(pos.lat, pos.lon, pos.alt, ground_track, ground_dist, dz)

        # 3) Energy/speed integration
        v_new_mps = self._integrate_energy_and_speed(pos, alt2, speed_mps, pitch_rad, n_drag, throttle, dt)
            ...
            if self.in_stall_recovery:
                ...
                if self.in_stall_recovery:
                    ...
                    if pitch_deg > self.stall_sink_pitch:
                        pitch_deg = self.update_pitch_deg(pitch_deg, self.stall_sink_pitch, speed_mps, dt, pos)
                        pitch_rad = math.radians(pitch_deg)
                        v_new_mps = self._integrate_energy_and_speed(pos, alt2, speed_mps, pitch_rad, n_drag, throttle, dt)
            else:
                # Not in explicit recovery: apply the soft energy guard band near stall (paper Eq. 21)
                alt_mid = 0.5 * (pos.alt + alt2)
                Ps = self.force_model.compute_specific_energy_rate(speed_mps, alt_mid, n_drag, throttle)
