...
def plot_aircraft_turn_rate() -> plt.Figure:
    ...
    # Plot components
    ax.plot(mach, turn_rate_ca_max, color='green', linewidth=2.0, linestyle='--',
            label=r'$\dot\chi(C_L^{\max})$')
    ax.plot(mach, turn_rate_n_max, color='red', linewidth=2.0, linestyle='--',
            label=r'$\dot\chi(n_{\max} = 9)$')
    ax.plot(mach, turn_rate_comp, color='blue', linewidth=2.5,
            label='Instantaneous Turn Rate')
