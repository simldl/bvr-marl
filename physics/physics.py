...
class AirLayer:
    ...
    def __init__(self, sea_level_density: float = SEA_LEVEL_DENSITY):
        ...
        self._alt_table = np.arange(0.0, self._ALT_MAX + self._ALT_STEP, self._ALT_STEP, dtype=np.float32)
    ...
    def get_density(self, alt_m: float) -> float:
        if 0.0 <= alt_m <= self._ALT_MAX:
            return float(np.interp(alt_m, self._alt_table, self._rho_table))
        return self._density_scalar(alt_m, self.rho0)

class ForceModel:
    def __init__(self, mass_kg: float, reference_area_m2: float, gravity: float, air: AirLayer,
                 cd0: float, K_ind: float, get_base_drag_cd, get_engine_force):
    ...
    def compute_specific_energy_rate(self, v_mps: float, alt_m: float, load_factor: float, throttle: float) -> float:
...
class PhysicsParams:
    ...
    stall0_mps: Optional[float] = None
    service_ceiling_m: Optional[float] = None
    stall_sink_pitch_deg: float = -5.0
    speed_full_thrust: float = 250.0
    stall_margin_mps: float = 10.0      # ΔV margin for guard band
    energy_relief_gain: float = 2.0     # k_E gain for pitch relief (keep 2.0)
    stall_recovery_min_time_s: float = 1.5  # <<< NEW: min time in recovery
