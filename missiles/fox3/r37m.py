...
class R37M(Missile):
    def __init__(self, firing_time_s, target, source, map_limits, group=None):
        ...
        config = {
            "mass_kg": 510.0,
            "reference_area_m2": 0.113,      # cross-section from 0.38 m diameter
            "aspect_ratio": 2.8,
            "oswald_e": 0.78,
            "drag_coefficient": 0.07,

            # Large boost-sustain surrogate for very long range
            "constant_engine_N": 20_000.0,
            "motor_burn_s": 20.0,

            "n_max": 30.0,                   # big missile; lower maneuver envelope
        ...
        super().__init__(
            ...
            config=config,
            data_link_mode="msl_support"      # likely requires mid-course support
        )
        self.fox_type = 3