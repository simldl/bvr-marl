class MissilePhaseManager:
    DEFAULT_PHASES = {
        ...
        # 'terminal' is post-burn; thrust_kN can remain in table for clarity, but is not used once terminal starts
        "terminal": {"duration_s": 50.0, "thrust_kN": 0.0},
    }

    def __init__(self, flight_phases: dict = None, life_time_s: float = None, motor_burn_s: float = None):
        if flight_phases is None:
            ...
            # powered duration must equal motor_burn_s
            flight_phases["middle"]["duration_s"] = max(0.0, float(motor_burn_s) - boost)

        self.flight_phases = flight_phases
        self.current_phase = "boost"
        # store powered duration end for scheduling
        self.motor_burn_s = float(motor_burn_s) if motor_burn_s is not None else (
            float(self.flight_phases["boost"]["duration_s"]) + float(self.flight_phases["middle"]["duration_s"])
        )
