...
def find_experiment_directory(checkpoint_path: str) -> Optional[str]:
    """
...
def process_checkpoint_resume(resume_checkpoint: Optional[str], project_root: str) -> Tuple[Optional[str], Optional[str]]:
    """
    ...
    if experiment_dir:
        ...
        print("=" * 80)
        print(f"No experiment directory found - loading from single checkpoint")
        print(f"CHECKPOINT: {os.path.basename(checkpoint_path)}")
        print(f"Will start NEW training with weights from this checkpoint")
        print("=" * 80)
        return None, checkpoint_path


def process_weight_loading(load_weights_from: Optional[str], project_root: str) -> Optional[str]:
    """
    ...
    if os.path.exists(checkpoint_path):
        print("=" * 80)
        print(f"STARTING NEW TRAINING WITH WEIGHTS FROM:")
