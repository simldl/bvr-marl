...
class EpisodeMetricsCallback(RLlibCallback):
    ...
    def on_episode_end(self, *, episode, env_runner, metrics_logger, env, env_index, rl_module, **kwargs):
        ...
        # Skip if no metrics_logger available
        ...
        if episode_info:
            ...
            if "team_a_mean_kills_per_agent" in first_agent_info:
                metrics_to_log["team_a_mean_kills_per_agent"] = first_agent_info["team_a_mean_kills_per_agent"]
                metrics_to_log["team_b_mean_kills_per_agent"] = first_agent_info["team_b_mean_kills_per_agent"]
                metrics_to_log["team_a_mean_deaths_per_agent"] = first_agent_info["team_a_mean_deaths_per_agent"]
                metrics_to_log["team_b_mean_deaths_per_agent"] = first_agent_info["team_b_mean_deaths_per_agent"]

            # Extract diagnostic metrics for missile firing
            if "team_a_valid_missile_shots" in first_agent_info:
                metrics_to_log["team_a_valid_missile_shots"] = first_agent_info["team_a_valid_missile_shots"]
                metrics_to_log["team_b_valid_missile_shots"] = first_agent_info["team_b_valid_missile_shots"]
                metrics_to_log["team_a_vetoed_missile_shots"] = first_agent_info["team_a_vetoed_missile_shots"]
                metrics_to_log["team_b_vetoed_missile_shots"] = first_agent_info["team_b_vetoed_missile_shots"]
                ...
                metrics_to_log["team_a_lock_ok_rate"] = first_agent_info["team_a_lock_ok_count"] / max(team_a_steps, 1)
                metrics_to_log["team_b_lock_ok_rate"] = first_agent_info["team_b_lock_ok_count"] / max(team_b_steps, 1)
                metrics_to_log["team_a_fov_ok_rate"] = first_agent_info["team_a_fov_ok_count"] / max(team_a_steps, 1)
                metrics_to_log["team_b_fov_ok_rate"] = first_agent_info["team_b_fov_ok_count"] / max(team_b_steps, 1)

            # Extract boundary death metrics
            if "team_a_boundary_deaths" in first_agent_info:
                metrics_to_log["team_a_boundary_deaths"] = first_agent_info["team_a_boundary_deaths"]
                metrics_to_log["team_b_boundary_deaths"] = first_agent_info["team_b_boundary_deaths"]
                metrics_to_log["total_boundary_deaths"] = first_agent_info["team_a_boundary_deaths"] + first_agent_info["team_b_boundary_deaths"]

            # Compute total metrics from team-level statistics
            metrics_to_log["total_missiles_fired"] = first_agent_info.get("team_a_missiles_fired", 0) + first_agent_info.get("team_b_missiles_fired", 0)
            metrics_to_log["total_kills"] = first_agent_info.get("team_a_kills", 0) + first_agent_info.get("team_b_kills", 0)
            metrics_to_log["total_deaths"] = first_agent_info.get("team_a_deaths", 0) + first_agent_info.get("team_b_deaths", 0)

            # Compute total normalized metrics (average across both teams)
            metrics_to_log["total_mean_kills_per_agent"] = first_agent_info.get("team_a_mean_kills_per_agent", 0) + first_agent_info.get("team_b_mean_kills_per_agent", 0)
            metrics_to_log["total_mean_deaths_per_agent"] = first_agent_info.get("team_a_mean_deaths_per_agent", 0) + first_agent_info.get("team_b_mean_deaths_per_agent", 0)

            # Log all metrics individually with window=1 to get true per-episode values
            # This ensures TensorBoard shows the average per episode, not accumulated values
