...
class WeaponFiringHandler:
    ...
    def handle_missile_firing(self, unit, action, selected_target, state, dt):
        ...
        if self.missile_auto.enable:
            ...
            if should_auto_fire:
                action[4] = 1.0
                logger.debug(f"Aircraft {aircraft_id}: AUTO_FIRE_TRIGGERED - target={getattr(selected_target, 'id', 'none')}")
        ...
        if trigger_result and selected_target is not None and missile_cooldown_ok and has_missiles:
            ...
            state['last_target_id'] = getattr(selected_target, 'id', None) if selected_target else None
            if veto_reason:
    ...
    def _fire_missile(self, unit, selected_target, state, aircraft_id):
        ...
        try:
            missile, veto, diagnostics = unit.weapons.fire_missile(
                self.simulator,
                selected_target, unit.missile_types[0]
            )
            ...
            if missile is not None:
                ...
                if missiles_at_target >= self.missile_auto.max_per_target:
                    ...
                    logger.info(f"Aircraft {aircraft_id}: MISSILE_FIRED_SUCCESS - LONG_COOLDOWN={self.missile_auto.long_cooldown_s}s")
                else:
                    self.weapon_cooldowns.set_missile_cooldown(state)
                    logger.info(f"Aircraft {aircraft_id}: MISSILE_FIRED_SUCCESS - missiles_at_target={missiles_at_target}")
