...
class EnergyProcessor:
    ...
    def process_energy_command(self, unit, action_Ps: float, state: dict, dt: float,
                               s_factor: float, c_factor: float) -> float:
        """
        ...
        # Get Ps limits
        Ps_min, Ps_max = self.energy_calculator.get_ps_limits(unit, v_bar, alt, n_bar, s_factor, c_factor)
    ...
    def compute_throttle_from_ps(self, unit, Ps_cmd: float, state: dict, n_factor: float) -> float:
        ...
        return self.energy_calculator.compute_energy_rate_throttle(
            unit, Ps_cmd, unit.speed, unit.position.alt,
            math.radians(unit.pitch_deg), n_factor
        )
