...
class MissileAutomation:
    ...
    def __init__(self, enable: bool = False, sqi_threshold: float = 0.3,
                 max_per_target: int = 2, long_cooldown_s: float = 10.0):
        """
    ...
    def configure(self, enable: bool = False, sqi_threshold: float = 0.3,
                 max_per_target: int = 2, long_cooldown_s: float = 10.0):
    ...
    def should_auto_fire(self, unit, selected_target) -> tuple:
        ...
        if target_id is not None:
            ...
            if missiles_at_target >= self.max_per_target:
                return (False, f"max_missiles_at_target({missiles_at_target}/{self.max_per_target})")
    ...
    def cleanup_destroyed_targets(self, active_unit_ids: set):
        ...
        targets_to_remove = [tid for tid in self.missiles_per_target.keys() if tid not in active_unit_ids]
