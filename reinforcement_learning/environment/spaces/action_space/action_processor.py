...
class ActionProcessor(ActionProcessorBase):
    ...
    - Action[0]: Ps - Specific energy rate command
    - Action[1]: n  - Normal load factor command
    - Action[2]: φ  - Bank angle command
    ...
    def __init__(self, simulator, use_speed_control=None, use_energy_space=None):
        ...
        self.weapon_handler = WeaponFiringHandler(self.missile_auto, self.weapon_cooldowns, self.trigger_proc)
    ...
    def apply(self, agent_id: int, action: np.ndarray):
        ...
        Ps_cmd = self.energy_proc.process_energy_command(unit, action[0], state, dt, s_factor, c_factor)
        new_throttle = self.energy_proc.compute_throttle_from_ps(unit, Ps_cmd, state, state['n_cmd_filtered'])

        alpha_T = self._get_alpha_dt_aware(dt, self.tau_T)
        state['throttle_filtered'] = (1 - alpha_T) * state['throttle_filtered'] + alpha_T * new_throttle
        unit.control.set_throttle(state['throttle_filtered'])

        self.lift_vector_proc.process_lift_vector_commands(unit, action[1], action[2], state, dt, s_factor, c_factor)

        # Target selection
        selected_target = self.target_sorter.select_target(unit, self.simulator, action[3], state, dt)
