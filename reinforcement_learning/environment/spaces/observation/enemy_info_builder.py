...
class EnemyInfoBuilder:
    ...
    def build(self, agent_id: str) -> Tuple[
        np.ndarray, np.ndarray,  # em_states, em_mask
        np.ndarray, np.ndarray,  # ef_states, ef_mask
    ]:
        """
        ...
        for tid, state, cov, tgt, utype, ref, confidence, n_obs, lifetime, update_count, is_deception, suspect_deception, engagement_id, jammer_id, engageable in tracks:
            ...
            if getattr(tgt, "is_missile", False):
                ...
                # Enemy fighters: add confidence (will add NEZ below)
                features = list(state[:6]) + [confidence]
                fighter_states.append(features)
                fighter_target_ids.append(tgt.id)

        # Get NEZ features from sensor (same source as BT)
        selected_target_id = getattr(unit.target, 'id', None) if hasattr(unit, 'target') and unit.target else None
        ...
        for i, tid in enumerate(fighter_target_ids):
            ...
            active_nez_val = nez_features.get('active_nez', 0.0) if tid == selected_target_id else 0.0
            ...
            # Append NEZ values to the fighter state
            enhanced_state = fighter_states[i] + [active_nez_val, passive_nez_val]
            enhanced_fighter_states.append(enhanced_state)

        # Pad to fixed size
        ef_arr, ef_mask = pad_generic(enhanced_fighter_states, self.config.ef_slots, self._ef_state_dim)
