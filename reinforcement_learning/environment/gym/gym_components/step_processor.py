...
class StepProcessor:
    ...
    def __init__(self, simulator: Simulator, obs_builder, action_processor,
                 reward_calculator, config):
    ...
    def process_step(self, actions: Optional[Dict[str, np.ndarray]], current_agents: list,
                    agent_to_unit_id: Dict[str, int], observation_space: Dict,
                    state_tracker, helpers, tick_secs: float) -> tuple:
        """
        ...
        tick_events = self.simulator.do_tick()
        kills_this_step = self._process_events(agent_to_unit_id, state_tracker)

        # Update missiles fired
        self._update_missiles_fired(current_agents, agent_to_unit_id, prev_missile_counts, state_tracker)

        # Build observations and compute rewards
        obs, rewards, terminateds, truncateds, infos = self._build_step_outputs(
            current_agents, agent_to_unit_id, observation_space,
            state_tracker, helpers, kills_this_step, tick_secs
        )
    ...
    def _apply_actions(self, actions: Optional[Dict], current_agents: list, agent_to_unit_id: Dict):
        """Apply actions to alive units."""
        actions = actions or {}
        for aid in current_agents:
            uid = agent_to_unit_id.get(aid)
    ...
    def _track_boundary_violations(self, current_agents: list, agent_to_unit_id: Dict, state_tracker):
        """Track boundary violations before simulation step."""
        for aid in current_agents:
            uid = agent_to_unit_id.get(aid)
            if uid is not None and uid in self.simulator.active_units:
    ...
    def _process_events(self, agent_to_unit_id: Dict, state_tracker) -> Dict:
        ...
        for event in new_events:
            if isinstance(event, UnitDestroyedEvent):
                self._process_kill_event(event, agent_to_unit_id, state_tracker, kills_this_step)
        return kills_this_step

    def _process_kill_event(self, event, agent_to_unit_id: Dict, state_tracker, kills_this_step: Dict):
        ...
        died_from_boundary = destroyed_agent_id in state_tracker.boundary_violators if destroyed_agent_id else False
        ...
        for aid, uid in agent_to_unit_id.items():
            if uid == killer_unit.id:
                state_tracker.record_kill(aid)
                kills_this_step[aid] = kills_this_step.get(aid, 0) + 1
                break
            elif hasattr(killer_unit, 'source') and hasattr(killer_unit.source, 'id') and killer_unit.source.id == uid:
                state_tracker.record_kill(aid)
                kills_this_step[aid] = kills_this_step.get(aid, 0) + 1
                break

    def _update_missiles_fired(self, current_agents: list, agent_to_unit_id: Dict,
                               prev_missile_counts: Dict, state_tracker):
        ...
        for aid in current_agents:
            ...
            if uid is not None and uid in self.simulator.active_units:
                ...
                state_tracker.update_missiles_fired(aid, fired)
            else:
                state_tracker.missiles_fired_last_step[aid] = 0

    def _build_step_outputs(self, current_agents: list, agent_to_unit_id: Dict,
                           observation_space: Dict, state_tracker, helpers,
                           kills_this_step: Dict, tick_secs: float) -> tuple:
        ...
        for aid in current_agents:
            ...
            if unit is None:
                ...
                reward_val, new_sqi, new_tactical_potential, new_energy_advantage = \
                    self._compute_agent_reward(aid, uid, unit, state_tracker, helpers,
                                              kills_this_step, tick_secs)
                rewards[aid] = reward_val

                # Update tracking state
                self.obs_info_builder.update_tracking_state(
                    aid, uid, unit, state_tracker, tick_secs,
                    new_sqi, new_tactical_potential, new_energy_advantage
                )
    ...
    def _compute_agent_reward(self, aid: str, uid: int, unit, state_tracker,
                             helpers, kills_this_step: Dict, tick_secs: float) -> tuple:
