...
class TerminationChecker:
    ...
    def check_termination(self, current_step: int, agent_to_unit_id: Dict[str, int],
                         agent_ids: List[str], opponent_ids: List[str]) -> Tuple[bool, bool, List[str], float]:
        """
    ...
    def compute_episode_info(self, end_reasons: List[str], agents_alive: bool, opponents_alive: bool,
                            current_step: int, state_tracker) -> Dict:
        """Compute episode-end information for logging."""
        # Compute team-level statistics
        total_missiles_fired_team_a = sum(state_tracker.episode_missiles_fired.get(aid, 0)
                                         for aid in self.config.agent_ids)
        total_missiles_fired_team_b = sum(state_tracker.episode_missiles_fired.get(aid, 0)
                                         for aid in self.config.opponent_ids)
        total_kills_team_a = sum(state_tracker.episode_kills.get(aid, 0) for aid in self.config.agent_ids)
        total_kills_team_b = sum(state_tracker.episode_kills.get(aid, 0) for aid in self.config.opponent_ids)
        total_deaths_team_a = sum(state_tracker.episode_deaths.get(aid, 0) for aid in self.config.agent_ids)
        total_deaths_team_b = sum(state_tracker.episode_deaths.get(aid, 0) for aid in self.config.opponent_ids)

        # Compute normalized statistics
        num_agents_team_a = len(self.config.agent_ids)
        num_agents_team_b = len(self.config.opponent_ids)
        mean_kills_per_agent_team_a = total_kills_team_a / num_agents_team_a if num_agents_team_a > 0 else 0.0
        mean_kills_per_agent_team_b = total_kills_team_b / num_agents_team_b if num_agents_team_b > 0 else 0.0
        mean_deaths_per_agent_team_a = total_deaths_team_a / num_agents_team_a if num_agents_team_a > 0 else 0.0
        mean_deaths_per_agent_team_b = total_deaths_team_b / num_agents_team_b if num_agents_team_b > 0 else 0.0

        # Compute diagnostic metrics
        total_valid_shots_team_a = sum(state_tracker.episode_valid_missile_shots.get(aid, 0)
                                      for aid in self.config.agent_ids)
        total_valid_shots_team_b = sum(state_tracker.episode_valid_missile_shots.get(aid, 0)
                                      for aid in self.config.opponent_ids)
        total_vetoed_shots_team_a = sum(state_tracker.episode_vetoed_missile_shots.get(aid, 0)
                                       for aid in self.config.agent_ids)
        total_vetoed_shots_team_b = sum(state_tracker.episode_vetoed_missile_shots.get(aid, 0)
                                       for aid in self.config.opponent_ids)

        # Count boundary deaths
        boundary_deaths_team_a = sum(1 for aid in self.config.agent_ids if aid in state_tracker.boundary_violators)
        boundary_deaths_team_b = sum(1 for aid in self.config.opponent_ids if aid in state_tracker.boundary_violators)
