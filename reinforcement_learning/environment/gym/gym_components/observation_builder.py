...
from reinforcement_learning.environment.gym.env_helpers import to_f32_obs, coerce_obs_to_space
...
class ObservationInfoBuilder:
    ...
    def build_observation(self, uid: int, observation_space) -> Dict:
        ...
        ob = to_f32_obs(self.obs_builder.build(uid))
        return coerce_obs_to_space(observation_space, ob)

    def build_terminal_observation(self, observation_space) -> Dict:
        """Build empty observation for terminated agent."""
        return coerce_obs_to_space(observation_space, {})
    ...
    def update_tracking_state(self, aid: str, uid: int, unit, state_tracker, tick_secs: float,
                             new_sqi: float, new_tactical_potential: float, new_energy_advantage: float):
        ...
        if has_lock:
            ...
            state_tracker.time_with_lock[aid] = state_tracker.time_with_lock.get(aid, 0.0) + tick_secs
        else:
        ...
        if hasattr(unit, "position"):
            state_tracker.update_position(aid, (
                unit.position.lon,
                unit.position.lat,
