...
class TargetManager:
        ...
        for candidate in candidates:
            try:
                ...
                candidate_rel = np.array([
                    ...
                    -(candidate.position.alt - self.aircraft.position.alt)  # negative because track uses -z
                ])
    ...
    def _get_target_candidates(self, simulator) -> List[Any]:
        ...
        for unit in simulator.active_units.values():
            if (unit.group != self.aircraft.group and
                not getattr(unit, "is_missile", False) and
                self._is_targetable(unit)):
    ...
    def _calculate_range_score(self, target) -> float:
        ...
        if hasattr(self.aircraft, 'wez') and callable(getattr(self.aircraft.wez, 'compute_dlz', None)):
            try:
    ...
    def _apply_automation_level_modifier(self, base_score: float, target, threats: List[Any]) -> float:
        ...
        if level == 'defensive':
            ...
            immediate_threat = any(getattr(t, 'distance_m', float('inf')) < 10000 
                                 for t in threats if getattr(t, 'source', None) == target)
            if immediate_threat:
