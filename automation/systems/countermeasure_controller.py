...
class CountermeasureController:
    ...
    def _get_radar_threats(self, threats: List[Any]) -> List[Any]:
        ...
        return [threat for threat in threats 
                if getattr(threat, 'threat_type', '') == 'radar_lock']
    ...
    def _estimate_time_to_impact(self, missile) -> float:
        ...
        try:
            ...
            if hasattr(missile, 'position') and hasattr(missile, 'velocity'):
                missile_pos = np.array([missile.position.lat, missile.position.lon, missile.position.alt])
                aircraft_pos = np.array([self.aircraft.position.lat, self.aircraft.position.lon, self.aircraft.position.alt])

                # Calculate range
                range_m = geodetic_distance_km(
                ...
                if hasattr(missile, 'airspeed_mps'):
                    ...
                    missile_speed = np.linalg.norm([missile.velocity.vx, missile.velocity.vy, missile.velocity.vz])
                else:
                ...
                if hasattr(self.aircraft, 'velocity'):
                    aircraft_vel = np.array([self.aircraft.velocity.vx, self.aircraft.velocity.vy, self.aircraft.velocity.vz])
                    missile_vel = np.array([missile.velocity.vx, missile.velocity.vy, missile.velocity.vz]) if hasattr(missile, 'velocity') else np.zeros(3)
    ...
    def _evaluate_flare_deployment(self, missile_threats: List[Any], current_time: float) -> Dict[str, bool]:
        """
        ...
        if hasattr(self.aircraft, 'sensor'):
            ...
            for missile in missile_warnings:
                ...
                if missile_type == 'fox2':
                    ...
                    if self.optimal_cm_window_min_s <= time_to_impact <= self.optimal_cm_window_max_s:
                        deploy['flares'] = True
                        return deploy
    ...
    def _evaluate_chaff_deployment(self, radar_threats: List[Any], current_time: float) -> Dict[str, bool]:
        """
    ...
    def _evaluate_ecm_deployment(self, radar_threats: List[Any], current_time: float) -> Dict[str, bool]:
    ...
    def _evaluate_decoy_deployment(self, threats: List[Any], current_time: float) -> Dict[str, bool]:
        ...
        high_threat_count = sum(1 for threat in threats 
                               if getattr(threat, 'threat_level', 'low') == 'high')
        
        if high_threat_count >= 2:  # Multiple high-priority threats
            deploy['decoys'] = True
            
        # Or if we're in a defensive automation mode and under serious threat
        if (self.config.automation_level == 'defensive' and 
            len(threats) >= self.config.threat_response_thresholds['multiple_threats']):
            deploy['decoys'] = True
