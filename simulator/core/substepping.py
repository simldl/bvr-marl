...
from simulator.core.helpers import units_distance_km, clamp
...
class Substepper:
    ...
    def __init__(self, config: Optional[SubstepConfig] = None) -> None:
    ...
    def run_tick_with_substeps(self, sim, tick_secs: float):
        ...
        # Check for hit calculator availability
        calc = None
        ccd = getattr(sim, "ccd", None)
        if ccd is not None:
            calc = getattr(ccd, "hit_calc", None)
        if calc is None:
            calc = getattr(sim, "hit_calc", None)
        
        # If we have a hit calculator, use it directly for each pair
        if calc is not None:
    ...
    def _integrate_temporarily_with_restore(self, sim, missile, target, tick_secs: float) -> None:
        ...
        while t_acc < tick_secs - 1e-12:
            ...
            if calc is not None:
                ...
                if hit and victim is not None and hasattr(sim, "ccd") and hasattr(sim.ccd, "on_hit"):
                    sim.ccd.on_hit(missile, victim, t_frac or 0.0, sim)
                    break
    ...
    def _estimate_substeps(self, missile, target, tick_secs: float) -> int:
        ...
        # Calculate substeps based on distance traveled per substep
    ...
    def _estimate_relative_speed_mps(self, a, b) -> float:
        # Prefer vector velocities
        for attr in ("vel_enu", "velocity"):
            va = getattr(a, attr, None)
            vb = getattr(b, attr, None)
            if va is not None and vb is not None:
                try:
                    ax = float(getattr(va, "vx", va[0])); ay = float(getattr(va, "vy", va[1])); az = float(getattr(va, "vz", va[2]))
                    bx = float(getattr(vb, "vx", vb[0])); by = float(getattr(vb, "vy", vb[1])); bz = float(getattr(vb, "vz", vb[2]))
                    dv0, dv1, dv2 = (ax - bx), (ay - by), (az - bz)
                    return (dv0*dv0 + dv1*dv1 + dv2*dv2) ** 0.5
                except Exception:
                    pass
        # Fallback: scalar speeds
