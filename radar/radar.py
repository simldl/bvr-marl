...
from radar.core.utils import to_cart, enu_to_geodetic
...
class Radar:
    def __init__(self,
        ...
        self.jam_susceptible = jam_susceptible  # True for aircraft radars, False for missile seekers
        # Force CPU for Ray workers to avoid CUDA conflicts with multiple workers
        force_cpu = os.environ.get("RADAR_FORCE_CPU", "0") == "1"
        self.device = torch.device("cpu") if force_cpu else torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # Get seed from owner ID, handle both int and string IDs
        ...
        self.lut = DetectionLUT(
            radar_frequency_hz, tx_power_w, 10 ** (antenna_gain_db / 10),
            max_range_m, snr_threshold_db,
            max_rcs=20.0, rcs_bins=lut_bins[1],
            dist_bins=lut_bins[0], device=self.device
        )
        self.obsgen = RadarObsGenerator(
            horizontal_fov_deg, vertical_fov_deg, max_range_m,
            self.lut, snr_threshold_db, false_alarm_rate,
            np_rng=self.np_rng, device=self.device
        )
    ...
    def update(self, tick_secs, sim, targets, owner_position, steer_h=0.0, steer_p=0.0, group_radars=None):
        ...
        if self.jam_susceptible and sim is not None and hasattr(sim, "ew_world"):
            ...
            # Degrade detections with per-angle jamming weighting
            ...
            for d in own_detections:
                ...
                # Sum weighted jamming from all jammers (per-detection angular weighting)
                J_total = 0.0
                for jam_az, jam_el, J_power in jammer_info:
                    # Angular separation between detection and jammer
                    angle_sep_deg = math.sqrt((det_az - jam_az)**2 + (det_el - jam_el)**2)
                    angle_sep_rad = math.radians(angle_sep_deg)

                    # Cosine taper: drops to zero at 90° (burn-through/sector effect)
                    overlap = max(0.0, math.cos(angle_sep_rad))
                    J_total += J_power * overlap

                # SNR degradation: SNR_eff = S/(N+J)
                ...
                if snr_eff_db < self.snr_threshold_db:
                    d["drop"] = True  # Mark for removal
                else:
                    d["snr_db"] = snr_eff_db  # Update effective SNR

            # Remove dropped detections
            own_detections = [d for d in own_detections if not d.get("drop", False)]

            # Inject deception ghosts from enemy jammers
            own_detections.extend(ghosts)

            # Ensure all detections have geodetic coordinates (lat/lon/alt)
            for d in own_detections:
                if ("lat" not in d) or ("lon" not in d) or ("alt" not in d):
                    enu_xyz = to_cart(d["az"], d["el"], d["d"])
                    lat, lon, alt = enu_to_geodetic(enu_xyz, owner_position.lat, owner_position.lon, owner_position.alt)
        ...
        # Enable self-triangulation: set export reference to owner's position (needs .lat, .lon, .alt)
    ...
    def _generate_and_transform_detections(self, owner_position, targets):
        ...
        detections = self.obsgen.generate(owner_position, targets, self.yaw_deg, self.pitch_deg, own_group=own_group, own_id=own_id)
    ...
    def _update_lock_ctrl(self, tracks):
        ...
        for tid, state, cov, tgt, utype, ref, confidence, n_obs, lifetime, update_count, is_deception, suspect_deception, engagement_id, jammer_id, engageable in tracks:
            # Only lock onto engageable targets (excludes suspected ghosts)
