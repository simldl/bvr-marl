...
def plot_f22_vs_f22() -> plt.Figure:
    ...
    # Add contour lines
    contours = ax1.contour(distances_km, rcs_values, prob_mesh.T, levels=[0.5, 0.9],
                           colors='black', linewidths=1.5, alpha=0.5)
    ax1.clabel(contours, inline=True, fontsize=24, fmt='P=%.1f')

    # Mark F-22 nominal RCS
    ax1.axhline(y=f22_rcs_nominal, color='blue', linestyle='--',
                linewidth=2, label=f'F-22 RCS (nominal: {f22_rcs_nominal:.4f} m²)')

    # Labels and title
    ax1.set_xlabel('Distance [km]', fontsize=28)
    ax1.set_ylabel('Radar Cross Section (RCS) [m²]', fontsize=28)
    ax1.set_title('Detection Probability Heatmap\n(F-22 Radar vs F-22 Target)',
    ...
    # Add contour lines
    contours2 = ax2.contour(distances_km, rcs_zoom, prob_zoom.T, levels=[0.5, 0.9],
                            colors='black', linewidths=1.5, alpha=0.5)
    ax2.clabel(contours2, inline=True, fontsize=24, fmt='P=%.1f')

    # Mark F-22 nominal RCS
    ax2.axhline(y=f22_rcs_nominal, color='blue', linestyle='--',
                linewidth=2, label=f'F-22 RCS (nominal: {f22_rcs_nominal:.4f} m²)')

    # Labels and title
    ax2.set_xlabel('Distance [km]', fontsize=28)
    ax2.set_ylabel('Radar Cross Section (RCS) [m²]', fontsize=28)
    ax2.set_title('Detection Probability Heatmap (Zoomed)\n(F-22 RCS Range)',
                  fontsize=32, fontweight='bold')
...
def plot_f22_vs_eurofighter() -> plt.Figure:
    ...
    ax1.axhline(y=eurofighter_rcs_nominal, color='red', linestyle='--',
    ...
    # Mark Eurofighter nominal RCS
    ax2.axhline(y=eurofighter_rcs_nominal, color='red', linestyle='--',
                linewidth=2, label=f'Eurofighter RCS (nominal: {eurofighter_rcs_nominal:.1f} m²)')
