...
def plot_rcs_altitude_angle() -> plt.Figure:
    ...
    for i, el_angle in enumerate(altitude_angles_deg):
        ...
        radar_pos = Position(
            lat=ref_pos.lat + (horiz_dist / 111000.0),
            lon=ref_pos.lon,
            alt=ref_pos.alt + vert_dist
        )
    ...
    ax.plot(altitude_angles_deg, rcs_f22, color='blue', linewidth=2.5,
            label=f'F-22 (σ₀={f22.rcs:.4f} m²)')
    ax.plot(altitude_angles_deg, rcs_f35, color='cyan', linewidth=2.0, linestyle='--',
            label=f'F-35 (σ₀={f35.rcs:.4f} m²)')
    ax.plot(altitude_angles_deg, rcs_su57, color='red', linewidth=2.0, linestyle='-.',
            label=f'Su-57 (σ₀={su57.rcs:.4f} m²)')
    ax.plot(altitude_angles_deg, rcs_eurofighter, color='green', linewidth=2.0, linestyle=':',
