...
from simulator.utils.angles import signed_yaw_deg_diff, yaw_geo_to_math
...
def enu_to_geodetic(enu_vec, ref_lat, ref_lon, ref_alt):
    ...
    # reference ECEF & rotation
    ref_xyz = geodetic_to_ecef(ref_lat, ref_lon, ref_alt)
    lat0 = np.radians(ref_lat); lon0 = np.radians(ref_lon)
    R = np.array([
...
def _effective_rcs(target: Unit, rp: Position) -> float:
    ...
    pitch = float(getattr(target, 'pitch_deg', getattr(getattr(target, 'control', None), 'pitch_deg', 0.0)))
    el_body = el_abs - pitch  # relative to body pitch (no roll available)

    # --- Pattern parameters (with safe defaults) ---
    ...
    # --- Azimuth multiplier f_az(φ) ---
    # Keep your legacy shape (front low → beam ~1 → tail moderately low), but smooth with sin^n.
    # Normalize sin(φ) in [0,1] for beam exposure; blend to distinct floors at front/tail.
    s = math.sin(math.radians(min(180.0, max(0.0, phi)))) ** n_az  # 0..1
    # Front/back weighting (linear by φ):
    w_front = max(0.0, 1.0 - phi / 180.0)  # 1 at 0°, 0 at 180°
    w_tail  = 1.0 - w_front                # 0 at 0°, 1 at 180°
    # Beam peak around 90° → 1.0; approach front/tail floors near 0/180
    f_az = (w_front * front_floor) + (w_tail * tail_floor) + (1.0 - w_front - w_tail) * 1.0
    # Enhance beam with s, but keep floors:
    f_az = max(min(f_az + (1.0 - max(front_floor, tail_floor)) * s, 1.0), min(front_floor, tail_floor))

    # --- Elevation multiplier f_el(ϑ) ---
    sev = (abs(math.sin(math.radians(el_body))) ** n_el)  # 0..1
    if el_body >= 0.0:
        # looking from above (dorsal) → modest gain
        f_el = 1.0 + k_top * sev
    else:
        # looking from below (ventral) → typically larger gain (inlets, bays)
        f_el = 1.0 + k_bottom * sev

    sigma = base_rcs * f_az * f_el
    # Clamp to physical-ish bounds relative to σ0
...
def _angles_dist(rp: Position, h: float, p: float, tp: Position) -> Tuple[float, float, float]:
    ...
    # Absolute geodetic angles
    az_abs = math.degrees(math.atan2(dE, dN))                   # 0°=N, +CW to E
    el_abs = math.degrees(math.atan2(dU, max(hd, 1e-12)))       # +up

    # Relative (signed) angles for FOV checks
    az_rel = signed_yaw_deg_diff(h, az_abs)                     # keep sign!
    el_rel = el_abs - p
...
def _doppler(target: Unit, az_abs_deg: float, el_abs_deg: float, freq_hz: float) -> float:
    ...
    r_hat = np.array([math.cos(er) * math.sin(ar),   # E
