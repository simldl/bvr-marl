class BaseLockController:
    ...
    def is_confirmed(self, tid):
        ...
        return entry is not None and entry[0] >= self._confirm_needed and entry[1] < self._miss_needed

    def locked_target_ids(self):
        return [tid for tid, (nconf, nmiss) in self._lock_state.items() if nconf >= self._confirm_needed and nmiss < self._miss_needed]
