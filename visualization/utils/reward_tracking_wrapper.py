        ...
        for agent_id in self.env.all_agent_ids:
            ...
            if unit is not None:
                try:
                    ...
                    enemies = [u for u in self.env.simulator.active_units.values()
                              if hasattr(u, 'group') and u.group != unit.group
                              and hasattr(u, 'position')]

                    targets = getattr(unit, 'locked_targets', []) if hasattr(unit, 'locked_targets') else []

                    incoming_missiles = [m for m in self.env.simulator.active_units.values()
                                        if hasattr(m, 'target') and m.target == unit]

                    # Get previous state info from env tracking (correct attribute names)
                    previous_position = self.env.previous_positions.get(agent_id)

                    missiles_fired = self.env.missiles_fired_last_step.get(agent_id, 0)
                    ...
                    # Get Phase 2 tracking state from environment
                    prev_sqi = self.env.prev_sqi.get(agent_id, 0.0)
                    nez_steps = self.env.nez_steps.get(agent_id, 0)
                    no_lock_duration_s = self.env.no_lock_timer.get(agent_id, 0.0)
                    orbit_state = self.env.orbit_state.get(agent_id, {})
                    tick_secs = self.env.tick_secs
                    time_with_lock_s = self.env.time_with_lock.get(agent_id, 0.0)
