...
from air_to_air_rl.visualization.scenplotter.video_generation import (
...
# ps_cmd_filtered  ← action_Ps  ∈ [0,1]  mapped to  [Ps_min, Ps_max]
#   We use a representative symmetric range; clamped to [0,1] after normalisation.
_PS_MIN, _PS_MAX = -150.0, 150.0  # m/s
# n_cmd_filtered   ← action_n   ∈ [0,1]  mapped to  [n_min=-2, n_max≈9]
_N_MIN, _N_MAX = -2.0, 9.0  # g
# phi_cmd_filtered ← action_phi ∈ [0,1]  mapped to  [-phi_max, +phi_max] with phi_max=45°
...
def _normalize_cmd(cmd: str, value: float) -> float:
    if cmd == "ps":
        return _norm(value, _PS_MIN, _PS_MAX)
    if cmd == "n":
        return _norm(value, _N_MIN, _N_MAX)
    if cmd == "phi":
...
class RLCommandsVisualizer:
    ...
    def _update_2d_sim(self, env: BVRMultiAgentEnv, timestep: int):
        ...
        drawables.append(
            TopLeftMessage(text=f"Step: {timestep}\nDuration: {env.simulator.elapsed_time_s:.1f} s")
        )
    ...
    def _update_rl_commands(self, env: BVRMultiAgentEnv):
        ...
        fighter_ids_seen = []
        for unit in env.simulator.active_units.values():
            if not isinstance(unit, Aircraft) or not self._is_fighter(unit):
                continue
