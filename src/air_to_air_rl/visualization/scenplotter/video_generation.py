...
from air_to_air_rl.visualization.scenplotter.scenario_plotter import Missile as DrawableMissile
...
def plot_missile(unit: Missile, side: str, trace_record: dict, active_unit_ids: set | None = None):
...
def plot_radar_cone(unit: Aircraft, side: str, enemy_rcs: float | None = None):
...
def _try_create_symbol_registry(mode: str = "nato"):
    ...
    if mode == "procedural":
        return None
    try:
        from air_to_air_rl.visualization.scenplotter.symbol_registry import SymbolRegistry
...
def live_simulation(
    ...
    display_limits = getattr(env, "full_map_limits", env.map_limits)
    ...
    # live playback and flushed to disk at episode end, so the saved video
    # always matches exactly what was shown on screen.
    _frame_buffer: list = []
    ...
    def update_frame(_frame_index):
        ...
        for _u in env.simulator.active_units.values():
            if isinstance(_u, Aircraft) and not getattr(_u, "is_support_asset", False):
        ...
        if save_video:
            ...
            # Flush after the very last frame so the video is written even if
            # the episode never terminates within the frame budget.
            if len(_frame_buffer) >= frames:
                _flush_frame_buffer(video_output_file)
        return (img,)

    if save_video:
        video_output_file.parent.mkdir(parents=True, exist_ok=True)
        print("Video recording enabled — frames captured live, saved at each episode end.")
        print(f"Output directory: {video_output_file.parent}")

        # Also flush on window close (catches mid-episode user close).
        def _on_close(_event):
            if _frame_buffer:
                print("Window closed — saving remaining frames …")
                _flush_frame_buffer(video_output_file)
