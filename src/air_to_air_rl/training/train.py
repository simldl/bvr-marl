...
Heavy lifting is delegated to focused modules in the training/ directory.
"""
...
# Set environment variables before importing Ray/torch
...
from air_to_air_rl.rl.environment.gym.bvr_multi_agent_env import BVRMultiAgentEnv  # noqa: E402
...
from air_to_air_rl.rl.utils import create_env_creator  # noqa: E402
from air_to_air_rl.utils.simple_config import (  # noqa: E402
    apply_overrides,
    load_config,
)
...
def train_main(cfg: dict, adaptive_config=None):
    """Core training logic."""
    ...
    if platform.system() == "Windows":
        ...
        ray_tmp = str(Path(tempfile.mkdtemp(prefix="air2air_ray_")).resolve())
    ...
    model_name = cfg["logging"]["model_name"] if "logging" in cfg else "bvr_model"
    print("=" * 80)
    ...
    ppo_config = build_ppo_config(
        cfg, obs_spaces, act_spaces, policy_mapping_fn, EpisodeMetricsCallback
    )
...
def main():
    """Main entry point for air2air-train command."""
    parser = argparse.ArgumentParser(description="Air-to-Air RL Training")
    parser.add_argument(
        "--config", "-c", type=str, help="Path to config file or config name (default: standard)"
    )
    parser.add_argument(
        "--config-path",
        type=str,
        help="Path to config directory (default: configs/training/)",
    )
    ...
    if args.config and ("/" in args.config or "\\" in args.config):
        ...
        config_path = str(full_config_path.parent)
    ...
    if args.adaptive:
        ...
        try:
            ...
            if adaptive_trainer is None:
                ...
                return train_with_fallback(cfg_dict, args.no_fallback)
            ...
            return train_with_adaptive_fallback(adaptive_cfg, adaptive_trainer, args.no_fallback)
...
def train_with_fallback(cfg_dict: dict, no_fallback: bool = False):
    """Train with fallback capability."""
    try:
        train_main(cfg_dict)
        return 0
    except Exception as e:
        ...
        try:
            ...
            return train_with_adaptive_fallback(cfg_dict, adaptive_trainer, no_fallback)
...
def train_with_adaptive_fallback(base_cfg: dict, adaptive_trainer, no_fallback: bool = False):
    """Train with adaptive configuration and automatic fallback on failure."""
    ...
    for attempt in range(max_attempts):
        try:
            if attempt == 0:
                ...
                print(
                    f"Training attempt {attempt + 1}/{max_attempts} with tier: {adaptive_trainer.current_tier}"
                )
            else:
                ...
                print(
                    f"Training attempt {attempt + 1}/{max_attempts} with fallback tier: {adaptive_trainer.current_tier}"
                )
            ...
            print(f"Training succeeded with configuration tier: {adaptive_trainer.current_tier}")
            return 0
