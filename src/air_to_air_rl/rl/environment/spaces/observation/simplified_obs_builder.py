...
Reads truth data directly from air_to_air_rl.simulator.active_units — no radar pipeline required.
...
from air_to_air_rl.rl.environment.spaces.obs_space_manager import SimplifiedEnvConfig
from air_to_air_rl.rl.environment.spaces.observation.missile_warning_builder import (
...
class SimplifiedObservationBuilder:
    ...
    def build(self, unit_id: str) -> dict[str, np.ndarray]:
        ...
        # Own state
        remaining_missiles = (
            unit.max_missiles - len(unit.missiles)
            if hasattr(unit, "max_missiles") and hasattr(unit, "missiles")
            else 0.0
        )
