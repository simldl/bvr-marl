...
class EnergyCalculator:
    ...
    def compute_energy_rate_throttle(
        self,
        unit,
        Ps_cmd: float,
        current_speed: float,
        ...
        T_max = (
            float(unit.physics.get_engine_force(v_safe, alt, 1.0))
    ...
    def get_ps_limits(
        self, unit, v_bar: float, alt: float, n_bar: float, s_factor: float, c_factor: float
    ) -> tuple:
        """
        ...
        T_max = (
            unit.physics.get_engine_force(v_bar, alt, 1.0)
            if hasattr(unit.physics, "get_engine_force")
            else 0.0
        )
