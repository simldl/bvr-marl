...
class BVREnvConfig:
    ...
    num_pr: int = 2
    num_warn: int = 4
    own_state_dim: int = 22  # 26 minus DLZ range redundancy, plus Ps
    ...
    def from_dict(cls, config: dict) -> BVREnvConfig:
        ...
        # Full display map limits: extends EW to accommodate AWACS figure-8 side zones.
        # Each side zone is 2 × orbit_radius_km wide (circle diameter).
        # NS extent stays equal to the combat zone (AWACS figure-8 fits within NS limits).
        awacs_cfg_pre = config.get("scenario_config", {}).get("awacs_config", {})
        awacs_radius_km = float(awacs_cfg_pre.get("orbit_radius_km", 25.0))
        ew_extension_km = 2.0 * awacs_radius_km  # extension per EW side
        half_ew = (map_size_km / 2.0 + ew_extension_km) / 111.0
