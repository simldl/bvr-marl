...
class StepProcessor:
    ...
    def process_step(
        ...
        self._update_missiles_fired(
            current_agents, agent_to_unit_id, prev_missile_counts, state_tracker
        )
    ...
    def _process_kill_event(
        self, event, agent_to_unit_id: dict, state_tracker, kills_this_step: dict
    ):
        ...
        died_from_boundary = (
            destroyed_agent_id in state_tracker.boundary_violators if destroyed_agent_id else False
        )
    ...
    def _update_missiles_fired(
        self, current_agents: list, agent_to_unit_id: dict, prev_missile_counts: dict, state_tracker
    ):
            ...
            if unit is None:
                ...
                reward_val, new_sqi, new_tactical_potential, new_energy_advantage = (
                    self._compute_agent_reward(
                        aid, uid, unit, state_tracker, helpers, kills_this_step, tick_secs
                    )
                )
                rewards[aid] = reward_val
