...
class BVRMultiAgentEnv(BaseMultiAgentEnv):
    ...
    def __init__(self, config: dict, model: Policy | None = None):
        ...
        # Initialize observation space from obs_space_mgr (required for Ray's new API stack)
        # This provides a proper initial observation space before reset is called
        try:
            self.observation_space = self.obs_space_mgr.all()
        except Exception:
            # Fallback: create a dummy observation space if obs_space_mgr fails
            self.observation_space = self.episode_manager.create_dummy_observation_space()
            if self.observation_space:
                # Create per-agent spaces from the dummy space
                self.observation_space = {
                    aid: self.observation_space for aid in self.config.all_agent_ids
                }
    ...
    def step(self, actions: dict[str, np.ndarray] | None = None):
        ...
        if (
            self.config.fix_radar_lock_after_first_obs
            and not self.episode_manager.radar_locks_fixed
        ...
        if terminated or truncated:
            ...
            episode_info = self.termination_checker.compute_episode_info(
                end_reasons, agents_alive, opponents_alive, self.current_step, self.state_tracker
            )
