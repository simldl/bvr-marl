...
class SmartCheckpointCallback(tune.Callback):
    ...
    def __init__(self, total_iterations, checkpoint_dir):
        ...
        print(
            f"  - Milestone iterations: {milestone_samples}... (every 5%, total: {len(self.milestones)})"
        )
        print("  - Will keep: ALL milestones + last 3 non-milestone backups")
        print("  - Time-based backups noted every 30 minutes (for monitoring only)")
    ...
    def on_trial_result(self, iteration, trials, trial, result, **info):
                ...
                if os.path.exists(cp_path):
                    try:
                        ...
                        self.all_checkpoints = [
                            (it, path) for it, path in self.all_checkpoints if path != cp_path
                        ]
        ...
        if deleted_count > 0:
            kept_milestone_count = len(
                [it for it in checkpoints_to_keep if it in self.milestone_checkpoints]
            )
            kept_backup_count = len(
                [it for it in checkpoints_to_keep if it not in self.milestone_checkpoints]
            )
            print(
                f"[Checkpoint] Deleted {deleted_count} old checkpoint(s). "
