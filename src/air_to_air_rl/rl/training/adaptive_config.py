    ...
    def get_system_info() -> dict[str, Any]:
        ...
        info = {
            "cpu": {
                ...
                "percent_used": psutil.cpu_percent(interval=1),  # Current CPU usage
            },
            "memory": {
...
class AdaptiveTrainingConfig:
    ...
    def get_adaptive_config(self, target_tier: str | None = None) -> dict[str, Any]:
        ...
        if target_tier is None:
            ...
            recommendations = tier_configs.get(tier, tier_configs["minimal"])
        ...
        print(
            f"CPU allocation: {total_threads} total threads, {reserved_threads} reserved, {available_for_workers} available for workers"
        )
        print(f"Current CPU usage: {cpu_usage:.1f}%")
        print(
            f"Recommended workers: {recommendations['num_env_runners']}, actual workers: {actual_workers}"
        )
        print(
            f"Expected total CPU usage after training starts: ~{actual_workers + reserved_threads} threads"
        )
        ...
        print(
            f"Environment configuration: {adapted_config['env'].get('environment_class', 'BVRMultiAgentEnv')}"
        )
        ...
        model_defaults = {
            "model_config": {
                ...
                "use_advanced_encoder": False,
                "encoder_type": "mlp",
                "use_tactical_lstm": True,
                "tactical_lstm_hidden": 128,
                "tactical_lstm_num_layers": 1,
                "tactical_num_modes": 3,
                "max_seq_len": 64,
            },
        }
    ...
    def get_fallback_config(self) -> dict[str, Any] | None:
        ...
        # Enhanced fallback hierarchy with API stack compatibility
        fallback_strategies = [
            # First try disabling new API stack with current tier (API compatibility issue)
            {"tier": self.current_tier, "force_simplified": False, "use_legacy_api": True},
            # Try lower tiers with same environment
            {"tier": "medium", "force_simplified": False, "use_legacy_api": False},
            {"tier": "low", "force_simplified": False, "use_legacy_api": False},
            # Try simplified environment with current tier
            {"tier": self.current_tier, "force_simplified": True, "use_legacy_api": False},
            # Try simplified with legacy API
            {"tier": self.current_tier, "force_simplified": True, "use_legacy_api": True},
            # Finally try minimal with simplified environment and legacy API
        ...
        if self.fallback_attempts <= len(fallback_strategies):
            ...
            # Force legacy API if strategy requires it
            ...
            # Force simplified environment if strategy requires it
    ...
    def print_system_summary(self):
        ...
        print(
            f"Memory: {info['memory']['total_gb']} GB total, {info['memory']['available_gb']} GB available ({info['memory']['percent_used']:.1f}% used)"
        )
...
def create_adaptive_trainer(config_path: str, auto_detect: bool = True) -> AdaptiveTrainingConfig:
    ...
    if auto_detect:
        ...
        # Ask user if they want to proceed with recommended config
        try:
            response = (
...
def save_adaptive_config(config: dict[str, Any], output_path: str, tier: str):
    """Save an adaptive configuration to a file."""
