...
class Threat:
    ...
    estimated_time_to_impact: float | None  # Seconds until impact (if applicable)
...
class ThreatAssessment:
            ...
            for contact_id, contact_data in radar_contacts.items():
                ...
                distance = (
                    geodetic_distance_km(
                        ...
                        self.aircraft.position.lon,
                        self.aircraft.position.alt,
                        source_unit.position.lat,
                        source_unit.position.lon,
                        source_unit.position.alt,
                    )
                    * 1000
                )  # Convert to meters
    ...
    def _assess_aircraft_threats(self, simulator) -> list[Threat]:
        ...
        for unit in simulator.active_units.values():
            if (
                ...
                or getattr(unit, "is_missile", False)
                or not hasattr(unit, "missiles")
            ):
                continue
    ...
    def _assess_gun_threats(self, simulator) -> list[Threat]:
        ...
        for unit in simulator.active_units.values():
            if unit.group == self.aircraft.group or getattr(unit, "is_missile", False):
                continue
            ...
            threat = Threat(
                ...
                bearing_deg=bearing,
                closure_rate_mps=closure_rate,
                estimated_time_to_impact=distance / max(closure_rate, 1.0)
                if closure_rate > 0
                else None,
                confidence=0.8,
            )
    ...
    def _classify_missile_threat(
        self, missile, distance: float, time_to_impact: float | None
    ) -> str:
    ...
    def _update_threat_history(self, current_threats: list[Threat]):
        ...
        for threat in current_threats:
            ...
            if threat_id not in self.threat_history:
                ...
                if levels.get(threat.threat_level, 0) > levels.get(
                    self.threat_history[threat_id]["max_threat_level"], 0
                ):
