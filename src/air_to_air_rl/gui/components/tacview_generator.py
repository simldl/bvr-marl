...
Generates Tacview ACMI scenarios via scripts/tacview/generate_scenario.py,
which delegates to the installed `air2air-tacview` / `air2air-tacview-bt` commands.

Supported arguments (from generate_scenario.py):
  --controller rl|behavior-tree
  --checkpoint PATH        (rl only)
  --model-config PATH      (rl only)
  --train-config PATH      (rl only)
  --aircraft F22|...       (behavior-tree only)
  --frames N
  --acmi PATH
  --seed N
  --num-scenarios N
...
from air_to_air_rl.services.tacview import GENERATE_SCRIPT, build_tacview_cmd
...
def _render_tacview_card(tp: ProcessRecord, monitor: TacviewProcessMonitor, is_dead: bool):
    ...
    with col_stop:
        if not is_dead:
            if st.button("⏹️ Stop", key=f"tv_stop_{tp.pid}", use_container_width=True):
    ...
    with col_open:
        if st.button("📂 Open", key=f"tv_open_{tp.pid}", use_container_width=True):
            if log_path.exists():
                if os.name == "nt":
...
def _render_monitor():
    ...
    with col2:
        if st.button("🔄 Refresh", key="tv_refresh", use_container_width=True):
            st.rerun()
    with col3:
        if st.button("🧹 Cleanup", key="tv_cleanup", use_container_width=True):
...
def find_checkpoint_files():
    from air_to_air_rl.services.visualization import find_checkpoint_files as _find
...
def _launch(label: str, extra_args: list, monitor: TacviewProcessMonitor):
    """Build command, launch subprocess via service, register with monitor."""
    if not Path(GENERATE_SCRIPT).exists():
        st.error(f"Script not found: `{GENERATE_SCRIPT}`")
        return

    import sys

    cmd = [sys.executable, GENERATE_SCRIPT] + extra_args
...
def tacview_generator():
    ...
    st.info(
        "Generates `.acmi` scenario files via **`scripts/tacview/generate_scenario.py`**, "
        "which calls the `air2air-tacview` (RL) or `air2air-tacview-bt` (behavior tree) "
        "package commands."
    )
    ...
    if not Path(GENERATE_SCRIPT).exists():
        st.error(
            f"⚠️ Script not found: `{GENERATE_SCRIPT}` — generation will fail until this exists."
        )

    # Active process monitor
    with st.expander("🖥️ Active Tacview Processes", expanded=True):
        _render_monitor()
    ...
    with tab1:
        ...
        with col2:
            ...
            if st.button(
                "🎬 Generate RL Scenario", type="primary", use_container_width=True, key="btn_rl"
            ):
                ...
                if rl_acmi:
                    args += ["--acmi", rl_acmi]
                label = f"RL_{rl_num}sc"
                _launch(label, args, monitor)

            st.markdown("---")
            st.markdown("#### Quick Launch")

            if st.button(
                "🎯 Quick Demo (random actions)", use_container_width=True, key="btn_rl_demo"
            ):
                _launch(
                    "RL_demo",
                    ["--controller", "rl", "--frames", "500", "--num-scenarios", "1"],
                    monitor,
                )

            if selected_ckpt and st.button(
                "🧠 Selected Checkpoint (1 scenario)", use_container_width=True, key="btn_rl_latest"
            ):
                _launch(
                    "RL_latest",
                    [
    ...
    with tab2:
        ...
        with col2:
            ...
            if st.button(
                "🌳 Generate BT Scenario", type="primary", use_container_width=True, key="btn_bt"
            ):
                args = [
                    "--controller",
                    "behavior-tree",
                ...
                if bt_acmi:
                    args += ["--acmi", bt_acmi]
                _launch(f"BT_{bt_aircraft}_{bt_num}sc", args, monitor)

            st.markdown("---")
            st.markdown("#### Quick Launch")

            if st.button(
                "🎲 Random BT Scenarios (3×)", use_container_width=True, key="btn_bt_random"
            ):
                _launch(
                    "BT_random_3",
                    [
            ...
            if st.button("⚔️ Single BT Scenario", use_container_width=True, key="btn_bt_single"):
                _launch(
                    "BT_single",
                    ["--controller", "behavior-tree", "--frames", "500", "--num-scenarios", "1"],
                    monitor,
                )
