...
def find_checkpoint_files():
    ...
    from air_to_air_rl.services.visualization import find_checkpoint_files as _svc_find
...
def training_dashboard():
    ...
    **📁 Training Configuration Files**
    Configs are loaded from `configs/training/` (project) or the package defaults.
    This ensures consistent paths and prevents config file confusion.
    """)

    # Create tabs for different training modes
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Active Training Monitor", "Standard Training", "Simplified Training", "Batch Training"]
    )
    ...
    with tab2:
        ...
        with col1:
            ...
            if config_files:
                ...
                st.warning("⚠️ No BVR configuration files found in `configs/training/`")
                st.markdown(
        ...
        with col2:
            ...
            if st.button("🔍 Validate Architecture", use_container_width=True):
                if not final_config:
            ...
            if st.button("📂 Open Models Folder", use_container_width=True):
                from air_to_air_rl.core.paths import project_root as _pr

                models_dir = _pr() / "models"
    ...
    with tab3:
        ...
        **Simplified Training Features:**
        ...
        with col2:
            if st.button(
                "🔍 Validate Architecture", key="validate_simple", use_container_width=True
            ):
            ...
            if st.button("🚀 Start Simplified Training", type="primary", use_container_width=True):
                if not simple_config:
...
def launch_batch_training_seeds(config, seeds):
    ...
    with st.spinner("Launching batch training..."):
        from air_to_air_rl.services.batch import build_batch_jobs, run_batch_jobs

        build_batch_jobs(base_config=config, seeds=seeds)
        [sys.executable, "-m", "air_to_air_rl.services.batch", "--config", config, "--seeds"] + [
        ...
        try:
            process = subprocess.Popen(
                [
                    sys.executable,
                    "-c",
                    f"from air_to_air_rl.services.batch import build_batch_jobs, run_batch_jobs; "
