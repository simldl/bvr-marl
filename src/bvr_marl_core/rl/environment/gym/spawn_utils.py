...
def spawn_position(
    group: str, map_size_km: float, map_limits, rng: np.random.Generator | None = None
) -> Position:
    """
    ...
    lat = float(generator.uniform(bottom_lat, top_lat))
    if group == "agent":
        lon = float(generator.uniform(left_lon, center_lon))
    elif group == "opponent":
        lon = float(generator.uniform(center_lon, right_lon))
    else:
        lon = float(generator.uniform(left_lon, right_lon))
    ...
    alt = float(generator.uniform(alt_low, alt_high))
...
def spawn_unit(env, aid: str, group: str) -> int:
    ...
    streams = getattr(getattr(env, "simulator", None), "random_streams", None)
    spawn_rng = streams.generator("spawn", aid) if streams is not None else None
    pos = spawn_position(group, map_size_km, env.map_limits, rng=spawn_rng)
    ...
    _apply_sensor_overrides(unit, cfg)

    # 7) Apply scripted scenario runtime flags
    _apply_scripted_opponent_flags(unit, group, cfg)

    # 8) Register into the simulator and start tracing
...
def _apply_sensor_overrides(unit, cfg: dict) -> None:
    """Apply env-level radar measurement-noise / data-link latency overrides.

    Lets the imperfect-information benchmark inject degradation into the causal
    sensor pipeline (radar report noise and one-way data-link report delay) via env
    config rather than only through aircraft-type defaults. No-op when the keys are
    absent, so nominal runs are byte-for-byte unchanged. (Data-link *dropout* is
    applied separately on the simulator via ``datalink_drop_prob``.)
    """
    radar = getattr(unit, "radar", None)
    if radar is None:
        return
    obsgen = getattr(radar, "obsgen", None)
    if obsgen is not None:
        if "radar_meas_angular_noise_deg" in cfg:
            obsgen.meas_angular_noise_deg = float(cfg["radar_meas_angular_noise_deg"])
        if "radar_meas_range_noise_m" in cfg:
            obsgen.meas_range_noise_m = float(cfg["radar_meas_range_noise_m"])
    base = cfg.get("radar_dl_delay_base_s")
    per_km = cfg.get("radar_dl_delay_per_km_s")
    if base is not None:
        radar.dl_delay_base_s = float(base)
    if per_km is not None:
        radar.dl_delay_per_km_s = float(per_km)
    if base is not None or per_km is not None:
        radar._dl_delay_enabled = radar.dl_delay_base_s > 0.0 or radar.dl_delay_per_km_s > 0.0
    # Noise-jamming: make this platform a jammer with the given burn-through range so
    # the EW world denies range to opposing radars (see radar.ew.ew_world). Applied to
    # every unit (symmetric EW), so opposing jammers degrade the agent's own picture.
    jammer_km = cfg.get("noise_jammer_burn_through_km")
    if jammer_km is not None:
        unit.noise_jammer_burn_through_km = float(jammer_km)
    # Missed-scan: per-scan own-detection dropout so tracks coast between updates.
    missed_scan = cfg.get("radar_missed_scan_probability")
    if missed_scan is not None:
        radar.missed_scan_probability = float(missed_scan)
    # Track coast: extra missed-update tolerance so coasting tracks persist and their
    # estimates diverge further from a manoeuvring target.
    coast_extra = cfg.get("track_coast_extra_updates")
    tracker = getattr(radar, "tracker_manager", None)
    if coast_extra is not None and tracker is not None:
        tracker.coast_extra_updates = max(0, int(coast_extra))


def _apply_scripted_opponent_flags(unit, group: str, cfg: dict) -> None:
    """Attach runtime invariants for scripted warmup opponents."""
    scenario_cfg = cfg.get("scenario_config", {}) or {}
    opponent_behavior = str(scenario_cfg.get("opponent_behavior", "policy") or "policy")
    if group == "opponent" and opponent_behavior in {"anchored_hold", "stationary_hold"}:
        unit.keep_inside_boundary = True
        if hasattr(unit.position, "copy"):
            unit.scripted_anchor_position = unit.position.copy()
    # Missile immunity for the boundary-survival warmup: the agent cannot lock,
    # target, or kill the opponent (and, like AWACS, it is dropped from the enemy
    # observation). Mirrors awacs_config.awacs_non_engageable.
    if group == "opponent" and bool(scenario_cfg.get("opponent_non_engageable", False)):
        unit.is_non_engageable = True
...
def _apply_spawn_variation(env, speed: float, pos, entity_id: object = "episode"):
    """Opt-in per-spawn randomisation of start speed and altitude.

    Enabled only when the corresponding config ranges are present, so existing
    scenarios (which set neither) keep their fixed starts. Uses numpy's global RNG,
    from the episode's named per-entity stream.
    """
    cfg = getattr(env, "config", {}) or {}
    streams = getattr(getattr(env, "simulator", None), "random_streams", None)
    rng = (
        streams.generator("spawn_variation", entity_id)
        if streams is not None
        else np.random.default_rng(0)
    )
    s_min, s_max = cfg.get("spawn_speed_min"), cfg.get("spawn_speed_max")
    if s_min is not None and s_max is not None:
        speed = float(rng.uniform(float(s_min), float(s_max)))
    a_min, a_max = cfg.get("spawn_alt_min"), cfg.get("spawn_alt_max")
    if a_min is not None and a_max is not None and pos is not None:
        pos.alt = float(rng.uniform(float(a_min), float(a_max)))
    return speed, pos
...
def spawn_with_geometry_config(env, geometry_data: dict) -> dict[str, int]:
    ...
    for i, aid in enumerate(agent_ids):
        if i < len(geometry_data["agent_positions"]):
            ...
            streams = getattr(getattr(env, "simulator", None), "random_streams", None)
            rng = streams.generator("spawn", aid) if streams is not None else None
            pos = spawn_position("agent", env.map_size_km, env.map_limits, rng=rng)
            heading = 90.0
            speed = 250.0

        speed, pos = _apply_spawn_variation(env, speed, pos, aid)
    ...
    for i, oid in enumerate(opponent_ids):
        if i < len(geometry_data["opponent_positions"]):
            ...
            streams = getattr(getattr(env, "simulator", None), "random_streams", None)
            rng = streams.generator("spawn", oid) if streams is not None else None
            pos = spawn_position("opponent", env.map_size_km, env.map_limits, rng=rng)
            heading = 270.0
            speed = 250.0

        speed, pos = _apply_spawn_variation(env, speed, pos, oid)
...
def spawn_awacs(
    ...
    The orbit centre is placed at orbit_distance_km from the map centre. If that
    places the AWACS beyond the combat zone, full_map_limits expands to keep the
    whole patrol pattern visible and boundary-safe.
    ...
    Args:
        ...
        lat_offset_deg: North/south offset of this platform's orbit centre from the
                        EW axis, used to separate multiple AWACS of the same team.
        name_suffix: Appended to the unit name to disambiguate paired AWACS.
        radar_overrides: Optional per-scenario radar retuning passed to the AWACS.
    ...
    desired_center_km = max(orbit_distance_km, 0.0)
    desired_center_lon_deg = desired_center_km / km_per_deg

    awacs_lat = float(lat_offset_deg)  # NS offset separates paired AWACS
    if group == "agent":
    ...
    awacs_unit = AWACScls(
        ...
        min_alt_m=min_alt,
        max_alt_m=max_alt,
        name=f"AWACS_{group.upper()}{name_suffix}",
    ...
    # Separate rule from the one above: this one hides the unit from hostile SENSORS
    # rather than refusing a launch at it. See AWACSConfigData.awacs_sensor_invisible.
    awacs_unit.is_sensor_invisible = bool(getattr(awacs_config, "awacs_sensor_invisible", True))

    # Create orbit controller centred on the AWACS spawn position
    streams = getattr(env.simulator, "random_streams", None)
    orbit_rng = (
        streams.generator("controller.awacs_orbit", f"{group}:{name_suffix or 'primary'}")
        if streams is not None
        else np.random.default_rng(0)
    )
    orbit_controller = create_orbit_controller(
        ...
        trail_fighters=getattr(awacs_config, "trail_fighters", False),
    ...
    if getattr(awacs_config, "trail_fighters", False) and hasattr(orbit_controller, "recenter"):
