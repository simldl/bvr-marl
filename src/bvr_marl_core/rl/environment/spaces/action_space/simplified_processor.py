...
Action vector (4 dims, all in [0, 1]):
  [0] energy_cmd   - specific excess power command; 0.5 = Ps 0 / hold energy
  [1] lift_phi     - lift-vector roll angle command; 0.5 = wings level
  [2] lift_N       - load-factor command; 0.5 = 1g neutral lift
...
class SimplifiedActionProcessor(ActionProcessorBase):
    ...
    Differences from ActionProcessor:
      ...
      - Centered flight controls: 0.5 means hold energy, wings level, and 1g.
    """
    ...
    def apply(self, agent_id: str, action: np.ndarray):
        ...
        self.lift_vector_proc.process_lift_vector_commands(
            unit, action[2], action[1], state, dt, s_factor, c_factor
        )
