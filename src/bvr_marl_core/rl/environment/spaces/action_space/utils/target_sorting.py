...
class TargetSorter:
    ...
    def __init__(self, tau_hold_s: float = 2.0, max_missiles_per_target: int = 0):
        ...
        Args:
            ...
            max_missiles_per_target: Saturation cap. When > 0, a target that
                already has this many friendly missiles inbound is removed from
                the selectable set (as long as an unsaturated bandit remains), so
                fighters spread their shots across targets instead of dumping a
                redundant missile on an already-saturated one. 0 disables the
    ...
    def select_target(self, unit, simulator, action_target: float, state: dict, dt: float):
        ...
        # Target allocation: drop fully-saturated bandits from the selectable set
        # when an unsaturated one remains, so a fighter cannot lock (and waste a
        # shot on) a target the team has already maxed out — it is steered to the
        # un-engaged bandit instead. Falls back to all candidates if every target
        # is saturated (e.g. 1v1).
        if self.max_missiles_per_target > 0:
            unsaturated = [
                u
                for u in raw_candidates
                if inbound_counts.get(getattr(u, "id", None), 0) < self.max_missiles_per_target
            ]
            if unsaturated:
                raw_candidates = unsaturated
