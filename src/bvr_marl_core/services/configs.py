...
def list_training_configs() -> list[str]:
    ...
    Delegates to :func:`bvr_marl_core.services.training.list_training_configs`
    so there is a single source of truth.
    """
    from bvr_marl_core.services.training import list_training_configs as _list
...
def list_visualization_configs() -> list[str]:
    ...
    Searches (in order):
    1. ``configs/visualization/`` at the core project root
    2. The package ``bvr_marl_core/visualization/configs/`` directory
    """
    from bvr_marl_core.utils.paths import core_root, visualization_configs_root
    ...
    dirs = [
        ...
        core_root() / "visualization" / "configs",
    ]
    for d in dirs:
        if d.is_dir():
            for ext in ("*.yaml", "*.yml"):
...
def resolve_training_config(name_or_path: str | Path) -> Path:
    ...
    from bvr_marl_core.utils.config_loader import resolve_training_config as _r
...
def load_gui_config() -> dict:
    ...
    from bvr_marl_core.utils.config_loader import load_gui_config as _load
...
def load_shared_config(name: str) -> dict:
    ...
    from bvr_marl_core.utils.config_loader import load_shared_config as _load
