...
class MapLimits:
    ...
    def max_latitude_extent_km(self):
        d1 = Geodesic.WGS84.Inverse(
            self.bottom_lat, self.left_lon, self.top_lat, self.left_lon, outmask=Geodesic.DISTANCE
        )
        d2 = Geodesic.WGS84.Inverse(
            self.bottom_lat, self.right_lon, self.top_lat, self.right_lon, outmask=Geodesic.DISTANCE
        )
    ...
    def max_longitude_extent_km(self):
        ...
        d2 = Geodesic.WGS84.Inverse(
            self.top_lat, self.left_lon, self.top_lat, self.right_lon, outmask=Geodesic.DISTANCE
        )
        return max(d1["s12"] / 1000.0, d2["s12"] / 1000.0)
    ...
    def in_boundary(self, lat, lon, alt):
        return bool(
            self.left_lon <= lon <= self.right_lon
            and self.bottom_lat <= lat <= self.top_lat
            and self.min_alt <= alt <= self.max_alt
        )
