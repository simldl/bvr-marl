...
def signed_yaw_deg_diff(actual: float, desired: float) -> float:
    delta_rad = math.atan2(math.sin(deg2rad(desired - actual)), math.cos(deg2rad(desired - actual)))
...
def yaw_geo_to_math(yaw_deg: float) -> float:
    """
    Convert geographic yaw (0° = North, 90° = East, ...)
    to mathematical angle (0° = East, 90° = North, ...) for motion vectors.
    """
...
def yaw_math_to_geo(hdg_deg: float) -> float:
    """
    Convert mathematical angle (0° = East, 90° = North, ...)
    back to geographic yaw (0° = North, 90° = East, ...).
    """
