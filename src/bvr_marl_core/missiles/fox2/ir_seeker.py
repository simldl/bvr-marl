...
class InfraredSeeker:
    ...
    def _get_ir_signature(self, target, missile_pos) -> float:
        ...
        if aspect_angle < 45:
            aspect_factor = 2.0  # rear aspect: hot exhaust
        elif aspect_angle < 135:
            aspect_factor = 1.0  # side aspect
        else:
            aspect_factor = 0.5  # front aspect: cold intake
    ...
    def _get_aspect_angle(self, target, missile_pos) -> float:
        bearing_tgt_to_msl = geodetic_bearing_deg(
            target.position.lat, target.position.lon, missile_pos.lat, missile_pos.lon
        )
