...
from bvr_marl_core.physics.physics import (
    AIRSPEED_OF_SOUND,
    BasePhysics,
    ForceModel,
    PhysicsParams,
...
class FlyingPhysics(BasePhysics):
    def __init__(self, params: PhysicsParams):
        ...
        self.ground_min_climb_pitch_deg = getattr(params, "ground_min_climb_pitch_deg", 5.0)
    ...
    def _ground_proximity_pitch_floor(self, alt_m: float) -> float:
        """
        Floor pitch limit: prevents nose-down pitch near the ground and forces a
        positive climb pitch at/below ground level so the aircraft cannot remain
        motionless at alt=0 (pitch=0 gives dz=0 → permanent stuck state).

        Returns the minimum allowed pitch in degrees:
          alt <= 0          : +ground_min_climb_pitch_deg  (forced climb)
          0 < alt < taper   : linear from +min_climb to -90°
          alt >= taper      : -90° (unrestricted)
        """
        taper = self.ground_clearance_taper_m
        min_climb = self.ground_min_climb_pitch_deg
        if taper <= 0.0 or alt_m >= taper:
            return -90.0
        elif alt_m <= 0.0:
            return min_climb
        else:
            # Linear: min_climb at alt=0 → -90° at taper
            return min_climb + (-90.0 - min_climb) * (alt_m / taper)
