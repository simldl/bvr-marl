...
class Afterburner:
    ...
    def update_spool(self, dt: float, v_mps: float, alt_m: float) -> None:
        ...
        # numeric hygiene
        self._blend = (
            0.0 if self._blend < 1e-6 else (1.0 if self._blend > 1.0 - 1e-6 else self._blend)
        )
    ...
    def fuel_flow_kgps(self, thrust_N: float) -> float:
        """Specific fuel consumption interpolation for logging/telemetry."""
