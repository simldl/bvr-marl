"""
Public / baseline training script for BVR-MARL.

Uses SimplifiedMultiAgentEnv with Ray RLlib's default PPO models — no custom
neural-network architecture.  This is the same pipeline as the core bvr-marl
public release and is suitable for quick experiments or as a reproducible
baseline before switching to a full extended model stack.

To use the full custom-network pipeline instead, run ``bvr-train-simple``.

Entry point: bvr-train-public
"""
...
from bvr_marl_core.rl.environment.gym.simplified_env import (  # noqa: E402
...
def train_public_main(cfg: dict) -> None:
    """Core public training loop.

    Uses default RLlib PPO models with no private custom RLModule.
    """
    ...
    if platform.system() == "Windows":
        ...
        ray_tmp = str(Path(tempfile.mkdtemp(prefix="bvr_pub_", dir=_tmp_base)).resolve())
    else:
        ray_tmp = str(Path(tempfile.mkdtemp(prefix="bvr_public_ray_")).resolve())
    ...
    model_name = cfg.get("logging", {}).get("model_name", "bvr_public")
    print("=" * 80)
    print("  PIPELINE     : Public baseline (default RLlib PPO, no custom network)")
    ...
    # Persist the config alongside the checkpoint so the GUI can auto-detect
    # the correct environment type when loading this run later.
    ...
    if not experiment_dir:
        ...
        if load_weights_from:
            ...
            print("=" * 80)
            print("STARTING NEW PUBLIC TRAINING WITH RANDOM WEIGHTS")
...
def main() -> int:
    """Entry point for ``bvr-train-public``."""
    parser = argparse.ArgumentParser(
        description="BVR-MARL public baseline training (default RLlib PPO, SimplifiedMultiAgentEnv)"
    )
    parser.add_argument(
        "--config",
        "-c",
    ...
    try:
        train_public_main(config.to_dict())
        return 0
    except Exception as exc:
