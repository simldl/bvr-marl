...
class TrackFusion:
    ...
    def process_with_fusion(
        own_position, own_tracks, external_positions, external_radars, targets, dt
    ):
        fused_results = []
            ...
            for ext_pos, ext_radar in zip(external_positions, external_radars):
                ...
                for state, cov, tgt, utype, ref_pos in ext_tracks:
                    lat, lon, alt = enu_to_geodetic(
                        state[:3], ext_pos.lat, ext_pos.lon, ext_pos.alt
                    )
                    enu_transformed = geodetic_to_enu(
                        lat, lon, alt, own_position.lat, own_position.lon, own_position.alt
                    )
