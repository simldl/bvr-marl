...
class RadarParamPolicy:
    ...
    def get_params_for_mode(
        ...
        if mode == "search":
            ...
            h_fov_deg = (
                lock_h_fov_deg
                if lock_h_fov_deg is not None
                else max(self.min_h_fov_deg, 0.1 * self.search_h_fov_deg)
            )
            v_fov_deg = (
                lock_v_fov_deg
                if lock_v_fov_deg is not None
                else max(self.min_v_fov_deg, 0.1 * self.search_v_fov_deg)
            )
            gain_increase = (
                10.0 * np.log10(self.search_h_fov_deg / h_fov_deg)
                if gain_scaling is None
                else gain_scaling
            )
            ...
            lock_false_alarm = (
                ...
                else self.search_false_alarm_rate * 0.2
            )
            lock_update_rate_hz = (
                update_rate_hz if update_rate_hz is not None else self.search_update_rate_hz * 2.0
            )
    ...
    def get_params_for_mode_auto(
        ...
        fov = self.get_adaptive_fov(
            distance_m, lock_distance=lock_distance, search_fov=search_fov, lock_fov=lock_fov
        )
        ...
        upd_rate = self.search_update_rate_hz * (
            2.0 if (tracker_stable or distance_m < lock_distance) else 1.0
        )
    ...
    def update_dynamic_params(self, distance_m=None, tracker_stable=False):
        ...
        if mode == "auto" and distance_m is not None:
            params = self.get_params_for_mode_auto(
                distance_m=distance_m, tracker_stable=tracker_stable
            )
        else:
