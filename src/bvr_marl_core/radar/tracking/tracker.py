    ...
    def update_tracks(
        ...
        for tid, kf in self.tracks.items():
            if tid not in updated_tids:
                kf.predict(dt)
                kf.missed_updates = getattr(kf, "missed_updates", 0.0) + miss_inc
    ...
    def _update_or_create_tracks(self, associations, dt: float, own_yaw_deg: float | None = None):
        ...
        for tid, c, meas, n_obs, current_ref in associations:
            ...
            if tid in self.tracks:
                ...
                meta["last_meas"] = meas
                # Persist the target identity so the track still reports its target
                # (and stays engageable) across coast ticks where no cluster carries
                # it — build_track_outputs otherwise loses tgt when clusters is empty.
                if c.get("T", None) is not None:
                    meta["tgt"] = c["T"]
                    meta["unit_type"] = getattr(c["T"], "unit_type", meta.get("unit_type"))
                ...
                self.track_meta[tid] = {
                    ...
                    # Persist target identity across coast ticks (see existing-track path).
                    "tgt": c.get("T", None),
                    "unit_type": getattr(c.get("T", None), "unit_type", None),
                }
