...
def profile_simulation_steps(num_episodes=3, steps_per_episode=100, config_name="aggressive"):
    """
    ...
    for episode in range(num_episodes):
        ...
        for step in range(steps_per_episode):
            ...
            actions = {
                agent_id: env.action_space[agent_id].sample() for agent_id in base_env.all_agent_ids
            }
...
def profile_specific_components():
    ...
    # Load config via the canonical config loader
    from air_to_air_rl.rl.utils.env_creator import create_env_creator
    from air_to_air_rl.utils.config_loader import load_train_config
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="aggressive", help="Config file name (without .yaml)"
    )
