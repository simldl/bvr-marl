...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Batch air-to-air RL training operations",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train with multiple seeds
  python scripts/batch/batch_train.py --config base_config.yaml --seeds 42 123 456

  # Train with config variations
  python scripts/batch/batch_train.py --configs config1.yaml config2.yaml config3.yaml

  # Hyperparameter sweep
  python scripts/batch/batch_train.py --config base.yaml --sweep learning_rate 0.001,0.0005,0.0001
        """,
    )
