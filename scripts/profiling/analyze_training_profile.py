...
Usage:
    python scripts/profiling/analyze_training_profile.py
"""
...
def categorize_function(func_name: str, file_name: str) -> str:
    ...
    if any(
        x in file_name.lower() for x in ["simulator", "bvr_multi_agent_env", "aircraft", "missile"]
    ):
...
def analyze_training_profile():
    ...
    for func, data in list(stats.stats.items())[:20]:
        ...
        print(
            f"{func_name[:50]:<50} {file_name[-30:]:<30} {cum_percent:5.1f}% {cumtime:8.3f} {tottime:8.3f} {ncalls:8}"
        )
    ...
    for func, data in list(stats.stats.items())[:20]:
        ...
        print(
            f"{func_name[:50]:<50} {file_name[-30:]:<30} {self_percent:5.1f}% {tottime:8.3f} {ncalls:8}"
        )
