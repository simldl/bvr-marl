...
def analyze_profile_stats(stats_file="profiling_results/simulation_profile.stats"):
    ...
    for func_name, filename, cum_time, self_time, pct, ncalls in top_funcs:
        ...
        print(
            f"{func_name:<50} {filename:<30} {pct:>6.1f}%  {cum_time:>9.3f}  {self_time:>9.3f}  {ncalls:>9}"
        )
    ...
    for func_name, filename, avg_time, ncalls, total_time_f in avg_time_funcs[:20]:
        func_name = func_name[:48] if len(func_name) > 48 else func_name
        filename = filename[-28:] if len(filename) > 28 else filename
        print(
            f"{func_name:<50} {filename:<30} {avg_time * 1000:>9.2f}  {ncalls:>9}  {total_time_f:>9.3f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--stats",
        type=str,
        default="profiling_results/simulation_profile.stats",
