...
class MockMissileRadar:
    ...
    def __init__(self):
        ...
        # Lock control
        self.lock_ctrl = type("LockCtrl", (), {"set_mode": self._set_mode})()
...
class MockMissile:
    ...
    def __init__(
        self, firing_time_s=0.0, target=None, source=None, map_limits=None, group="BLUE", **kwargs
    ):
        ...
        if source and hasattr(source, "position"):
            self.position = MockPosition(
                source.position.lat, source.position.lon, source.position.alt
            )
        else:
            self.position = MockPosition()
    ...
    def update(self, tick_secs, sim):
        ...
        if self.target and hasattr(self.target, "position"):
            new_yaw, new_pitch = self.guidance.compute(
                self.yaw_deg, self.pitch_deg, self.position, self.target.position, tick_secs
            )
        ...
        result = self.physics.compute_movement(
            self.position, self.yaw_deg, self.pitch_deg, self.speed, thrust, tick_secs
        )
        ...
        if self.target and hasattr(self.target, "position"):
            ...
            if distance < 50.0:  # 50m proximity fuse
                ...
                return [
                    type(
                        "Event", (), {"type": "missile_hit", "missile": self, "target": self.target}
                    )()
                ]
...
class MockPosition:
    ...
    def distance_to(self, other):
        return math.sqrt(
            (self.lat - other.lat) ** 2 + (self.lon - other.lon) ** 2 + (self.alt - other.alt) ** 2
        )
