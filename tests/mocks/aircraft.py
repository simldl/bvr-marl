...
# Import the real Position class to ensure compatibility
from bvr_marl_core.simulator.core.helpers import Position
from tests.mocks.missiles import MockMissile


class MockPosition(Position):
    """Mock position class that inherits from real Position for compatibility."""
...
class MockPhysics:
    ...
    def compute_movement(
        self, position, current_yaw, desired_yaw, current_pitch, desired_pitch, speed, throttle, dt
    ):
...
class MockWeaponSystem:
    ...
    def select_and_engage_target(
        self, candidates, target_selector, fire_action, gun_fire_action, simulator
    ):
...
class MockControl:
    ...
    def update_movement(self, tick_secs):
        ...
        if hasattr(self.parent, "physics"):
            ...
            self.parent.speed = max(
                self.parent.min_speed_mps, min(speed, self.parent.max_speed_mps)
            )
