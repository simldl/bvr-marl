...
from simulator.utils.geodesics import geodetic_bearing_deg, geodetic_distance_km

# Uses your actual probability path:
#  - σ_eff from radar.core.utils._effective_rcs
#  - Pd = radar.lut.get_probability(dist, σ_eff)
from radar.core.utils import _effective_rcs  # σ_eff(az, el) model used by obsgen
# (LUT is reached via blue.sensor.radar.lut.get_probability)
# obsgen.generate uses exactly: p = lut.get_probability(dist, σ_eff) then torch.rand > p
# so we seed torch for reproducibility.  (see RadarObsGenerator.generate)
torch.manual_seed(1234)
np.random.seed(1234)

def _mk_aircraft(name, group, pos, yaw_deg, rcs, map_limits, extra_cfg=None):
    cfg = {
        "mass_kg": 18000, "reference_area_m2": 50, "max_speed_mps": 300, "n_max": 9,
        "radar_horizontal_fov_deg": 60, "radar_vertical_fov_deg": 30,
        "radar_max_range_m": 120_000, "radar_frequency_hz": 10e9,
        "radar_tx_power_w": 1.5e4, "radar_antenna_gain_db": 40,
        "radar_snr_threshold_db": 10, "rcs": rcs
    }
    if extra_cfg:
        cfg.update(extra_cfg)
    return Aircraft(
        name=name, position=pos, yaw_deg=yaw_deg, speed_mps=300.0,
        group=group, map_limits=map_limits, config=cfg
    )
...
def test_radar_detection_probabilities_match_lut_and_rcs(tmp_path):
    ...
    blue = _mk_aircraft(
        ...
        yaw_deg=0.0, rcs=10, map_limits=map_limits
    )
    sim.add_unit(blue)
    ...
    red_ns = _mk_aircraft(                                                  # non-stealth
        ...
        yaw_deg=(geodetic_bearing_deg(lat_ns, lon_ns, blue.position.lat, blue.position.lon) + 10.0) % 360.0,
    ...
    red_st = _mk_aircraft(                                                  # stealth
        ...
        yaw_deg=(geodetic_bearing_deg(lat_st, lon_st, blue.position.lat, blue.position.lon) - 10.0) % 360.0,
    ...
    az_to_mid = geodetic_bearing_deg(
        ...
        (lat_ns + lat_st) * 0.5, (lon_ns + lon_st) * 0.5
    )
    ...
    print(f"[INFO] Initial separations: NS={range_m(blue.position, red_ns.position)/1000:.2f} km, "
    ...
    for t in range(10):
        ...
        print(f"[t+{t+1:02d}] Pd(NS)={Pd_ns:.3f} (σ={sigma_ns:.3e}, R={R_ns/1000:.1f}km) | "
        ...
        if dets:
            ...
            for d in dets[:2]:
                ...
                if snr is not None:
                    print(f"  det: az={d['az']:.1f} el={d['el']:.1f} R={d['d']/1000.0:.1f}km  SNR={snr:.1f} dB")

        if trks:
            got_any_track = True
            for (tid, x6, P3, tgt, utype, ref,
                 conf, n_obs, life, upd_cnt,
                 is_dec, suspect_dec, eng_id, jam_id, engageable) in trks:
                print(f"  track tid={tid} conf={conf:.2f} n_obs={n_obs} life={life} upd={upd_cnt} "
    ...
    assert Pd_ns >= Pd_st + 0.2, "Expected significantly higher detection probability for non-stealth"
