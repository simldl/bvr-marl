...
from radar.core.utils import _effective_rcs  # your aspect-dependent σ_eff
...
def _range_m(a_pos: Position, b_pos: Position) -> float:
    return geodetic_distance_km(a_pos.lat, a_pos.lon, a_pos.alt, b_pos.lat, b_pos.lon, b_pos.alt) * 1000.0
...
def test_radar_pd_two_rcs_runs(map_limits):
    ...
    for idx, (label, rcs_val) in enumerate(scenarios):
        ...
        blue = _mk_aircraft(
            ...
            yaw_deg=0.0, rcs=10.0, map_limits=map_limits
        )
        sim.add_unit(blue)
        ...
        red = _mk_aircraft(
            ...
            yaw_deg=(geodetic_bearing_deg(lat_t, lon_t, blue.position.lat, blue.position.lon) + 10.0) % 360.0,
            rcs=rcs_val, map_limits=map_limits
        )
        sim.add_unit(red)
        ...
        for t in range(10):
            ...
            print(f"[{label} t+{t+1:02d}] σ0={red.rcs:.6f} m^2 | σ_eff={sigma_eff:.6e} m^2 | R={R/1000:.1f} km | Pd={Pd:.3f}")
            ...
            print(f"           detections={len(dets)} clusters={len(clus)} tracks={len(tracks or [])} locked={locked_ids}")
