...
class MockOwner:
    ...
    def __init__(self, x=0.0, y=0.0, z=0.0, unit_id=0):
        self.id = unit_id
        self.position = type('Pos', (), {'x': x, 'y': y, 'z': z, 'lat': 0.0, 'lon': 0.0, 'alt': z})()
...
class TestECMEmitterInitialization:
    ...
    def test_assist_sectors(self):
        ...
        jammer = ECMEmitter(
            ...
            erp_w=1000.0,
            bw_hz=1e6,
            assist_ahead_deg=60.0,
            assist_behind_deg=30.0
        )
...
class TestFrequencyHopping:
    ...
    def test_frequency_hopping_initialization(self):
        ...
        assert jammer._on_target == True  # Starts on target
...
class TestDeceptionGeneration:
    ...
    def test_generate_deception_with_drfm(self, basic_jammer, victim_radar):
        ...
        victim_pos = type('Pos', (), {'x': 5000.0, 'y': 0.0, 'z': 5000.0,
                                       'lat': 0.0, 'lon': 0.0, 'alt': 5000.0})()
    ...
    def test_deception_within_assist_sector(self, basic_jammer, victim_radar):
        ...
        victim_behind = type('Pos', (), {'x': -5000.0, 'y': 0.0, 'z': 5000.0,
                                          'lat': 0.0, 'lon': 0.0, 'alt': 5000.0})()

        ghosts_behind = basic_jammer.deception_burst(
            victim_radar=victim_radar,
            victim_position=victim_behind,
            t=0.0
        )
...
class TestECMEffectivenessScenarios:
    ...
    def test_multiple_jammers_additive_effect(self, victim_radar):
        ...
        # Create two jammers
        jammer1 = ECMEmitter(
            owner=MockOwner(x=1000.0, y=0.0, z=5000.0, unit_id=1),
            erp_w=1000.0,
            bw_hz=1e6
        )
        jammer2 = ECMEmitter(
            owner=MockOwner(x=-1000.0, y=0.0, z=5000.0, unit_id=2),
            erp_w=1000.0,
            bw_hz=1e6
        )
