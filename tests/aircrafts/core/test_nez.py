...
def test_kinematic_range_altitude_bonus(nez_calculator):
    ...
    range_high = nez_calculator._kinematic_range(
        missile, 300.0, 250.0, 0.0, 15000.0, 10000.0
    )
    
    # Same altitude baseline
    range_same = nez_calculator._kinematic_range(
        missile, 300.0, 250.0, 0.0, 10000.0, 10000.0
    )
...
def test_kinematic_range_closing_speed(nez_calculator):
    ...
    range_head_on = nez_calculator._kinematic_range(
        missile, 300.0, 250.0, 0.0, 10000.0, 10000.0
    )
    
    # Tail chase (low closing speed)
    range_tail = nez_calculator._kinematic_range(
        missile, 300.0, 250.0, 180.0, 10000.0, 10000.0
    )
...
def test_nez_visibility(nez_calculator, mock_target):
    ...
    assert nez_calculator.nez_visible(range_in_nez, dlz) == True
    assert nez_calculator.nez_visible(range_outside_nez, dlz) == False
...
def test_nez_calculator_error_handling():
    ...
    try:
        invalid_calc = NoEscapeZoneCalculator(None)
