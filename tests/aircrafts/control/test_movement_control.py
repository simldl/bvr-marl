...
import pytest
import math
from tests.mocks import MockAircraft, MockPosition, MockPhysics
...
def test_yaw_control_setter(control_system):
    ...
    assert abs(control.desired_yaw_deg - (-30.0)) < 0.1 or abs(control.desired_yaw_deg - 330.0) < 0.1
...
def test_pitch_control_setter(control_system):
    ...
    assert abs(control.desired_pitch_deg - (-10.0)) < 0.1 or abs(control.desired_pitch_deg - 350.0) < 0.1
...
def test_position_clamping_integration(control_system):
    ...
    assert aircraft.boundary_violation_active == True
...
def test_control_system_error_handling():
    ...
    try:
        invalid_control = AircraftControlSystem(None)
