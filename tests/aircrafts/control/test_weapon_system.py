...
from air_to_air_rl.aircrafts.control.weapon_system import AircraftWeaponSystem
...
def mock_aircraft():
    ...
    aircraft = MockAircraft(
        name="test_weapons", position=MockPosition(0, 0, 5000), yaw_deg=0.0, speed=250.0
    )
...
def test_target_selection_with_candidates(weapon_system, mock_target):
    ...
    if hasattr(weapon_system.parent.sensor, "select_target"):
        weapon_system.parent.sensor.select_target = lambda candidates, selector: (
...
def test_missile_firing_basic(weapon_system, mock_target):
    ...
    # Mock sensor methods needed for missile firing
    weapon_system.parent.sensor.has_radar_lock = lambda target: True
    weapon_system.parent.sensor.get_locked_targets = lambda: {mock_target.id: ([], 0.0)}
    ...
    if result[0] is not None:
        assert hasattr(result[0], "target")
        assert result[0].target == mock_target
    else:
        # Check if there was a valid reason for not firing
        assert result[1] is not None  # Should have a veto reason
...
def test_gun_firing_basic(weapon_system, mock_target):
    ...
    sim = MockSimulator()
    # Create a valid datetime object for gun system compatibility
    sim.utc_time = datetime.fromtimestamp(1000.0)  # Use positive timestamp
...
def test_gun_firing_invalid_inputs(weapon_system):
    ...
    try:
        weapon_system.fire_gun(None, None)
