...
from tests.mocks import MockAircraft, MockMissile, MockPosition, MockSimulator
...
def mock_aircraft():
    ...
    aircraft = MockAircraft(
        name="test_warned_aircraft", position=MockPosition(0, 0, 10000), yaw_deg=0.0, speed=300.0
    )
...
def missile_warner(mock_aircraft):
    ...
    return MissileWarner(parent=mock_aircraft, detection_delay_s=1.0, detection_delay_std=0.2)
...
def test_missile_warner_initialization(mock_aircraft):
    ...
    mw = MissileWarner(parent=mock_aircraft, detection_delay_s=2.0, detection_delay_std=0.5)
...
def test_is_pending_basic(missile_warner):
    ...
    assert missile_warner._is_pending("missile_001")
    assert not missile_warner._is_pending("missile_002")


def test_is_pending_empty(missile_warner):
    """Test pending check with no pending warnings."""
    assert not missile_warner._is_pending("any_missile")
...
def test_is_pending_multiple_missiles(missile_warner):
    ...
    missile_warner._pending_warnings.extend(
        [(15.0, "missile_001", None), (16.0, "missile_002", None), (17.0, "missile_003", None)]
    )

    assert missile_warner._is_pending("missile_001")
    assert missile_warner._is_pending("missile_002")
    assert missile_warner._is_pending("missile_003")
    assert not missile_warner._is_pending("missile_004")
...
def test_delay_randomization(mock_aircraft, mock_simulator, mock_missiles):
    ...
    mw = MissileWarner(parent=mock_aircraft, detection_delay_s=2.0, detection_delay_std=1.0)

    [m for m in mock_missiles if m.target == mock_aircraft][0]
...
def test_complete_warning_workflow(missile_warner, mock_simulator, mock_missiles):
    ...
    missile_warner.check_for_new_missiles(sim_time, mock_simulator)
    len(missile_warner._pending_warnings)
...
def test_missile_warner_extreme_delays(mock_aircraft):
    ...
    mw_large = MissileWarner(
        parent=mock_aircraft, detection_delay_s=1000.0, detection_delay_std=100.0
    )

    # Very small delay
    mw_small = MissileWarner(
        parent=mock_aircraft, detection_delay_s=0.001, detection_delay_std=0.0001
    )
