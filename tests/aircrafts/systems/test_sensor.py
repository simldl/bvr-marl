...
def mock_aircraft():
    ...
    aircraft = MockAircraft(
        name="test_sensor_aircraft", position=MockPosition(0, 0, 10000), yaw_deg=0.0, speed=300.0
    )
...
def test_sensor_system_performance(sensor_system, mock_simulator):
    ...
    for i in range(50):
        target = MockAircraft(
            name=f"target_{i}", position=MockPosition(i * 0.01, 0, 10000), speed=250.0
        )
