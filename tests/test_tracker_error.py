    ...
    while True:
        ...
        if 10 < dist_km < 100:
            ...
            if abs(((brg - shooter.yaw_deg + 180) % 360) - 180) < 30:
                break
    yaw = (geodetic_bearing_deg(lat, lon, shooter.position.lat, shooter.position.lon)
           + rng.uniform(-30, 30)) % 360
...
def test_tracker_focus(sim, map_limits):
    ...
    assert missile is not None, f"Missile creation failed. Veto: {veto_reason}, Diagnostics: {diagnostics}"

    print("\n=== TRACKER DIAGNOSTICS ===")
    print("Columns:")
    print("tick | dist_true_m | est_err_E_m est_err_N_m est_err_U_m | pos_err_m | "
    ...
    for tick in range(120):
        ...
        dEt, dNt, dUt = geodetic_to_enu(
            target.position.lat, target.position.lon, target.position.alt,
            prev_target_llh[0],  prev_target_llh[1],  prev_target_llh[2]
        )
        ...
        if est_pos is not None:
            dE_cur, dN_cur, dU_cur = geodetic_to_enu(
                est_pos.lat, est_pos.lon, est_pos.alt,
                target.position.lat, target.position.lon, target.position.alt
            )
        ...
        if isinstance(est_vel, (list, tuple, np.ndarray)) and len(est_vel) == 3:
            ...
            vel_is_outlier = (
                (v_est_norm > (v_true_mps + 200.0)) or
                (v_est_norm > 800.0 and last_valid_v_est is not None and
                 v_est_norm > 1.5 * float(np.linalg.norm(last_valid_v_est))) or
                freeze_next_tick
            )
        ...
        if pos_is_outlier and last_valid_est_pos_llh is not None:
            ...
            dE, dN, dU = geodetic_to_enu(
                last_valid_est_pos_llh[0], last_valid_est_pos_llh[1], last_valid_est_pos_llh[2],
                target.position.lat,       target.position.lon,       target.position.alt
            )
