...
class TestBVREnvConfigScenario:
    ...
    def test_from_dict_supports_rectangular_combat_zone(self):
        ...
        assert config.map_width_km == 400.0
        assert config.map_height_km == 200.0
        assert config.map_size_km == 400.0
        assert config.map_limits.left_lon == pytest.approx(-200.0 / 111.0)
        assert config.map_limits.right_lon == pytest.approx(200.0 / 111.0)
        assert config.map_limits.bottom_lat == pytest.approx(-100.0 / 111.0)
        assert config.map_limits.top_lat == pytest.approx(100.0 / 111.0)
        assert config.full_map_limits.left_lon == pytest.approx(-200.0 / 111.0)
        assert config.full_map_limits.right_lon == pytest.approx(200.0 / 111.0)

    def test_full_map_limits_expand_only_when_awacs_extend_beyond_combat_zone(self):
        config_dict = {
            "num_agents_per_side": 2,
            "map_width_km": 400,
            "map_height_km": 200,
    ...
    def test_full_map_limits_do_not_expand_when_awacs_disabled(self):
        ...
        assert config.map_limits.right_lon == pytest.approx(200.0 / 111.0)
        assert config.full_map_limits.right_lon == pytest.approx(config.map_limits.right_lon)
