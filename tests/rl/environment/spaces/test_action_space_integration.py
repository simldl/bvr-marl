...
class TestActionSpaceIntegration:
    ...
    def mock_simulator(self):
        ...
        sim.tick_secs = 0.1
        sim.active_units = {}
        sim.count_missiles_at_target = Mock(return_value=0)
        return sim
    ...
    def mock_unit(self):
        ...
        unit.physics.compute_total_drag = Mock(
            return_value=10000.0
        )  # Add this to fix the drag calculation
    ...
    def mock_target(self):
        ...
        target.is_non_engageable = False  # Ensure target is not filtered by non-engageable check
    ...
    def test_missile_fires_only_with_lock_and_fov(
        ...
        with patch.object(
            action_processor.target_sorter, "sort_target_candidates", return_value=[mock_target]
        ):
