...
from simulator.core.helpers import Position, units_distance_km
...
def random_target(map_limits, rng, missile_source):
    while True:
        ...
        dist_km = np.sqrt((lat - missile_source.position.lat)**2 + (lon - missile_source.position.lon)**2) * 111
        if 10 < dist_km < 100:
            tgt_bearing = geodetic_bearing_deg(missile_source.position.lat, missile_source.position.lon, lat, lon)
            angle_diff = abs(tgt_bearing - missile_source.yaw_deg)
            if angle_diff > 180: angle_diff = 360 - angle_diff
            if angle_diff < 30:
                break
    yaw = geodetic_bearing_deg(lat, lon, missile_source.position.lat, missile_source.position.lon) + rng.uniform(-45, 45)
    yaw = yaw % 360
    ...
    target = Aircraft(
        ...
        config={
            "mass_kg": 18000, "reference_area_m2": 50, "max_speed_mps": 600, "n_max": 9,
            "radar_horizontal_fov_deg": 60, "radar_vertical_fov_deg": 30,
            "radar_max_range_m": 120000, "radar_frequency_hz": 10e9, "radar_tx_power_w": 15e3,
            "radar_antenna_gain_db": 40, "radar_snr_threshold_db": 10, "rcs": 1000
        }
    )
    return target
...
def blue_aircraft(map_limits):
    ...
    return Aircraft(
        ...
        config={
            ...
            "radar_horizontal_fov_deg": 60, "radar_vertical_fov_deg": 30,
            "radar_max_range_m": 120000, "radar_frequency_hz": 10e9, "radar_tx_power_w": 15e3,
            "radar_antenna_gain_db": 40, "radar_snr_threshold_db": 10, "rcs": 10
        }
    )
...
def test_detailed_missile_tracking_and_guidance(sim, map_limits):
    ...
    from radar.core.utils import _angles_dist, enu_to_geodetic, geodetic_to_enu
    ...
    bearing = geodetic_bearing_deg(
        blue.position.lat, blue.position.lon,
    ...
    dist_km_init = geodetic_distance_km(
        blue.position.lat, blue.position.lon, blue.position.alt,
    ...
    assert missile is not None, f"Missile was not created! Veto: {veto_reason}, Diagnostics: {diagnostics}"
    ...
    print(f"[INFO] Extended motor burn from {original_motor_burn:.1f}s to {missile.motor_burn_s:.1f}s")
    ...
    lat, lon, alt = enu_to_geodetic(_enu, missile.position.lat, missile.position.lon, missile.position.alt)
    _ = geodetic_to_enu(lat, lon, alt, missile.position.lat, missile.position.lon, missile.position.alt)
    ...
    for t in range(170):
        ...
        dist_mt_m = geodetic_distance_km(
        ...
        traj["blue"]["x"].append(bx);     traj["blue"]["y"].append(by)
        traj["missile"]["x"].append(mx);  traj["missile"]["y"].append(my)
        traj["target"]["x"].append(tx);   traj["target"]["y"].append(ty)
        ...
        if not (target.id in sim.active_units and missile.id in sim.active_units):
            if target.id not in sim.active_units:
                print(f"[RESULT] Target removed -> count as HIT at tick {t+1} (dist≈{dist_mt_m:.1f} m)")
                hit = True
    ...
    def _mark_start_end(xs, ys, name):
        if xs and ys:
            ...
            ax.annotate(f"{name} start", (xs[0], ys[0]), xytext=(5,5), textcoords="offset points", fontsize=8)
            ax.annotate(f"{name} end",   (xs[-1], ys[-1]), xytext=(5,5), textcoords="offset points", fontsize=8)
