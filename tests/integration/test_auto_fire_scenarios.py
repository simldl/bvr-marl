...
def run_scenario(scenario_name, agent_config, enemy_config, threshold=0.3, max_steps=30):
    ...
    agent = Eurofighter(agent_pos, agent_hdg, agent_spd, "BLUE", map_limits, 0.0, 20000.0)
    ...
    enemy = Eurofighter(enemy_pos, enemy_hdg, enemy_spd, "RED", map_limits, 0.0, 20000.0)
    enemy.missile_types = [AIM120_AMRAAM]
    simulator.add_unit(enemy)

    # Calculate initial range
    from bvr_marl_core.simulator.utils.geodesics import geodetic_distance_km

    initial_range_km = geodetic_distance_km(
        agent_lat, agent_lon, agent_alt, enemy_lat, enemy_lon, enemy_alt
    )

    print("\n[SETUP]")
    print(
        f"  Agent: {agent_lat:.2f}°N, {agent_lon:.2f}°E, {agent_alt:.0f}m, hdg={agent_hdg:.0f}°, spd={agent_spd:.0f}m/s"
    )
    print(
        f"  Enemy: {enemy_lat:.2f}°N, {enemy_lon:.2f}°E, {enemy_alt:.0f}m, hdg={enemy_hdg:.0f}°, spd={enemy_spd:.0f}m/s"
    )
            ...
            if hasattr(agent, "metrics") and hasattr(agent.metrics, "get_sqi"):
                try:
                    ...
                    if sqi_result.get("valid", False):
                        ...
                        print(
                            f"  [STEP {step}] FIRED! SQI={sqi_at_fire:.3f}, total={missiles_fired}"
                        )
                except Exception:
                    pass
...
def test_all_scenarios():
    ...
    print("LEGEND")
    print("=" * 80)
    print("PASS = Fired max missiles (2) - Auto-fire triggered")
    print("PART = Fired some missiles (1) - Partial trigger")
    print("FAIL = No missiles fired - SQI below threshold (0.3)")
    ...
    fired_sqi = [
        r["initial_sqi"]
        for r in results
        if r["initial_sqi"] is not None and r["missiles_fired"] > 0
    ]
    not_fired_sqi = [
        r["initial_sqi"]
        for r in results
    ...
    if scenarios_fired >= 3:
        print("\nPASS Threshold 0.3 appears GOOD - triggers in favorable geometries")
    elif scenarios_fired >= 2:
        print("\nPART Threshold 0.3 appears REASONABLE - selective firing")
    else:
        print("\nFAIL Threshold 0.3 may be TOO HIGH - very conservative firing")
    ...
    # The function now verifies results instead of returning them
    # All assertions for expected behavior should be added here
    assert len(results) == 5, f"Expected 5 scenarios, got {len(results)}"

    # At least some scenarios should trigger firing (not all should be 0)
    scenarios_with_firing = sum(1 for r in results if r["missiles_fired"] > 0)
    assert scenarios_with_firing > 0, "At least some scenarios should trigger auto-fire"

    print("\nPASS All scenario tests completed successfully")


if __name__ == "__main__":
    test_all_scenarios()  # Uses assertions now
    sys.exit(0)
