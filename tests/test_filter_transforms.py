...
from radar.core.utils import geodetic_to_enu, enu_to_geodetic, to_cart
...
from radar.tracking.filter.constant_velocity_filter import ConstantVelocityKFFilter
...
def test_enu_rotation_properties(ref_A, ref_B):
    ...
    I = R.T @ R
    assert np.allclose(I, np.eye(3), atol=1e-9)
...
def test_delta_direction_matches_geodetic_to_enu(ref_A, ref_B):
    ...
    delta_expected = geodetic_to_enu(ref_A.lat, ref_A.lon, ref_A.alt,
                                     ref_B.lat, ref_B.lon, ref_B.alt)
...
def test_velocity_rotates_only(ref_A, ref_B):
    ...
    R = enu_rotation(ref_A.lat, ref_A.lon, ref_B.lat, ref_B.lon)
    delta = geodetic_to_enu(ref_A.lat, ref_A.lon, ref_A.alt,
                            ref_B.lat, ref_B.lon, ref_B.alt)
...
def test_cluster_fallback_vs_geodetic_short_range(ref_B):
    ...
    lat, lon, alt = enu_to_geodetic(np.array(to_cart(az_deg, el_deg, d_m)),
                                    src.lat, src.lon, src.alt)
