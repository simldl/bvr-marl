...
class TestMissileGuidance:
    ...
    def test_missile_guidance_import(self):
        ...
        from air_to_air_rl.missiles.guidance.guidance import MissileGuidance
    ...
    def test_missile_guidance_initialization(self, mock_missile, mock_target_provider):
        ...
        with patch.multiple(
            "air_to_air_rl.missiles.guidance.guidance",
            LoftGuidance=Mock(),
    ...
    def test_missile_guidance_compute_guidance(self, mock_missile, mock_target_provider):
        ...
        with (
            ...
            patch("air_to_air_rl.missiles.guidance.guidance.DirectPursuitGuidance") as mock_direct,
            patch("air_to_air_rl.missiles.guidance.guidance.PnPropNavGuidance") as mock_pn,
            patch(
                "air_to_air_rl.missiles.guidance.guidance.CircularMovingAverageFilter"
            ) as mock_circular,
            patch("air_to_air_rl.missiles.guidance.guidance.MovingAverageFilter") as mock_moving,
        ):
    ...
    def test_missile_guidance_target_in_fov(self, mock_missile, mock_target_provider):
        ...
        with (
            ...
            patch("air_to_air_rl.missiles.guidance.guidance._angles_dist") as mock_angles,
        ):
            ...
            in_fov = guidance._target_in_radar_fov(
                mock_missile.position, mock_target_provider.get_guidance_target()
            )
    ...
    def test_missile_guidance_filtering(self, mock_missile, mock_target_provider):
        ...
        with (
            ...
            patch(
                "air_to_air_rl.missiles.guidance.guidance.DirectPursuitGuidance"
            ) as mock_direct_filt,
            patch("air_to_air_rl.missiles.guidance.guidance.PnPropNavGuidance") as mock_pn_filt,
            patch(
                "air_to_air_rl.missiles.guidance.guidance.CircularMovingAverageFilter"
            ) as mock_circular_filt,
            patch(
                "air_to_air_rl.missiles.guidance.guidance.MovingAverageFilter"
            ) as mock_moving_filt,
        ):
    ...
    def test_radar_fov_does_not_double_apply_offsets(self, mock_missile, mock_target_provider):
        """
        _target_in_radar_fov() must NOT add yaw_offset_deg / pitch_offset_deg
        to radar.yaw_deg / radar.pitch_deg.

        radar.yaw_deg is a property that already returns owner.yaw_deg + yaw_offset_deg.
        Adding the offset a second time rotates the boresight reference by 2×offset,
        which causes targets near the edge of the FOV to be mis-classified.

        Regression test: with a 30° gimbal offset and a target placed exactly on the
        true boresight (missile_yaw + offset), the target must be detected as inside
        the FOV regardless of the offset magnitude.
        """
        from air_to_air_rl.missiles.guidance.guidance import MissileGuidance
        ...
        with patch.multiple(
            ...
            OFFSET = 30.0
            MISSILE_YAW = 90.0  # missile points East
            TRUE_BORESIGHT = MISSILE_YAW + OFFSET  # 120°

            # Simulate radar.yaw_deg property: already includes offset
            mock_missile.radar.yaw_deg = TRUE_BORESIGHT
            mock_missile.radar.pitch_deg = 0.0
            mock_missile.yaw_deg = MISSILE_YAW
            mock_missile.pitch_deg = 0.0
            mock_missile.radar.yaw_offset_deg = OFFSET
            mock_missile.radar.pitch_offset_deg = 0.0
            mock_missile.radar.h_fov_deg = 60.0
            mock_missile.radar.v_fov_deg = 30.0
            ...
            # Place target exactly on the true boresight (0° relative azimuth)
            # _angles_dist(missile_pos, h=TRUE_BORESIGHT, ...) → az≈0
            with patch("air_to_air_rl.missiles.guidance.guidance._angles_dist") as mock_angles:
                mock_angles.return_value = (0.0, 0.0, 5000.0)  # on-boresight
                in_fov = guidance._target_in_radar_fov(
                    mock_missile.position, mock_target_provider.get_guidance_target()
                )
                # Must be inside FOV (0° < h_fov/2 = 30°)
                assert in_fov is True, (
                    "Target on true boresight was classified as outside FOV — "
                    "radar offset is likely being double-applied."
                )
                # Verify _angles_dist was called with the already-offset yaw (TRUE_BORESIGHT),
                # not with TRUE_BORESIGHT + OFFSET (double-offset)
                called_yaw = mock_angles.call_args[0][1]
                assert called_yaw == pytest.approx(TRUE_BORESIGHT), (
                    f"_angles_dist called with yaw={called_yaw}, expected {TRUE_BORESIGHT}. "
                    "Offset appears to have been added twice."
                )
