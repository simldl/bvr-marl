...
class TestPnPropNavGuidance:
    ...
    def test_pn_propnav_import(self):
        ...
        try:
            ...
            pytest.skip("PnPropNavGuidance not available")
    ...
    def test_pn_propnav_initialization(self, mock_missile):
        ...
        try:
            ...
            assert guidance.pn_law == PnPropNavGuidance.PN_TRUE  # Default is PN_TRUE (1), not PN_PURE (2)
            assert guidance.ALTITUDE_THRESHOLD == 2000.0
            assert guidance.g == 9.80665
            assert guidance._prev_r is None
            
            # Test custom initialization
            guidance_custom = PnPropNavGuidance(
                mock_missile, 
                pn_n=5.0, 
                pn_law=PnPropNavGuidance.PN_TRUE, 
                altitude_threshold=3000.0
            )
            ...
            pytest.skip("PnPropNavGuidance dependencies not available")
    ...
    def test_pn_propnav_closing_velocity(self, mock_missile):
        ...
        try:
            ...
            Vc = max(1e-6, -float(np.dot(Vrel, Ulos)))  # -dot([100, -100, 0], [1, 0, 0]) = -100, but max(1e-6, -100) = 1e-6
            ...
            Vc_closing = max(1e-6, -float(np.dot(Vrel_closing, Ulos)))  # -dot([-300, 0, 0], [1, 0, 0]) = 300
    ...
    def test_pn_propnav_pure_pn_law(self, mock_missile, mock_missile_position, mock_target_position):
        """Test Pure PN law implementation."""
        try:
            from missiles.guidance.pn_propnav import PnPropNavGuidance
            
            with patch('missiles.guidance.pn_propnav.geodetic_to_enu') as mock_enu:
    ...
    def test_pn_propnav_true_pn_law(self, mock_missile, mock_missile_position, mock_target_position):
    ...
    def test_pn_propnav_target_velocity_integration(self, mock_missile, mock_missile_position, mock_target_position):
    ...
    def test_pn_propnav_velocity_estimation(self, mock_missile, mock_missile_position, mock_target_position):
        ...
        try:
            ...
            with patch('missiles.guidance.pn_propnav.geodetic_to_enu') as mock_enu:
                ...
                # First call - establish initial position
                mock_enu.return_value = [1000.0, 1000.0, 500.0]
                guidance.compute(0.0, 0.0, mock_missile_position, mock_target_position, 0.1)
                
                # Second call - target has moved
                mock_enu.return_value = [1100.0, 1050.0, 520.0]  # Target moved
                
                yaw, pitch = guidance.compute(0.0, 0.0, mock_missile_position, mock_target_position, 0.1)
                
                # Should estimate velocity and produce valid commands
                assert 0.0 <= yaw <= 360.0
                assert -89.0 <= pitch <= 89.0
                
                # Previous position should be stored
                assert guidance._prev_r is not None
                np.testing.assert_allclose(guidance._prev_r, [1100.0, 1050.0, 520.0], rtol=1e-6, atol=1e-12)
                
        except ImportError:
            pytest.skip("PnPropNavGuidance dependencies not available")

    def test_pn_propnav_angle_normalization(self, mock_missile, mock_missile_position, mock_target_position):
