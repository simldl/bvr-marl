...
Key findings from branch comparison (main vs scripted-behavior):
  ...
  - Remaining weaknesses (still in scripted-behavior branch):
      ...
      B. Phase selection: PN only activates when the target is inside the
         ...
         lateral mover can drift outside FOV, forcing fallback to fov_capture
      ...
      E. At long range (>30 km with F22 radar settings) the shooter's radar
         ...
         The missile is then stuck in fov_capture with no position at all.
...
class TestGuidancePhaseDistribution:
    """Verify PN phase is actually used (not just fov_capture/lead throughout)."""
    ...
    def test_pn_used_for_head_on(self):
        ...
        assert pn_frac > 0.3, (
            ...
            "Guidance may be stuck in fov_capture (no tracker data)."
        )
    ...
    def test_pn_or_lead_used_for_beam(self):
        ...
        assert pn_lead_frac > 0.2, (
            ...
            f"Full dist: {pd}. "
            "Missile stuck in fov_capture — velocity not being used."
        )
...
def _print_summary():
    ...
    print(
        ...
        INTERPRETATION:
          ...
          velocity, missile is stuck in fov_capture (position-only).
        """)
    )
