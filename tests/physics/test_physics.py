...
from physics.physics import (
    AirLayer, ForceModel, PhysicsParams, BasePhysics,
    SEA_LEVEL_DENSITY, STANDARD_GRAVITY, AIRSPEED_OF_SOUND
)
...
class TestAirLayer:
    ...
    @pytest.mark.parametrize("altitude, rho_min, rho_max", [
        (0,       1.10, 1.30),
        (5_000,   0.60, 0.90),
        (11_000,  0.25, 0.40),
        (20_000,  0.05, 0.15),
    ...
    def test_density_monotonic_decrease(self, air_layer):
        ...
        assert all(x > y for x, y in zip(densities, densities[1:])), "Density must decrease monotonically"
