...
import pytest
import math
from simulator.core.helpers import Position, units_distance_km, units_bearing, clamp
...
class TestHelpersCompatibility:
    """Test compatibility with existing helpers tests."""
    
    def test_position_copy_compatibility(self):
        """Test compatibility with existing position copy test."""
        pos = Position(51.0, 8.0, 700)
        copied = pos.copy()
        
        assert pos == copied and pos is not copied
    
    def test_units_distance_km_compatibility(self):
        """Test compatibility with existing distance test."""
        unit_a = MockUnit(51.0, 8.0, 700)
        unit_b = MockUnit(51.001, 8.001, 750)
        
        distance = units_distance_km(unit_a, unit_b)
        
        assert distance > 0
    
    def test_units_bearing_compatibility(self):
        """Test compatibility with existing bearing test."""
        unit_a = MockUnit(51.0, 8.0, 0)
        unit_b = MockUnit(52.0, 8.0, 0)  # North
