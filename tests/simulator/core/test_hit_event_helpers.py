...
from bvr_marl_core.simulator.core.hit_calculation import HitParams
...
class TestKillProbability:
    ...
    def test_stochastic_on_hit_uses_miss_distance(self):
        """A far miss with a forced low roll still does not kill; the missile is
        always consumed regardless."""
        missile = SimpleNamespace(id=1, hit_probability=1.0, lethal_radius_m=50.0)
        target = SimpleNamespace(id=2)
        active = {1: missile, 2: target}
        sim = SimpleNamespace(
            active_units=active,
            rnd_gen=SimpleNamespace(random=lambda: 0.5),  # mid roll
            remove_unit=lambda uid: active.pop(uid, None),
            log_event=lambda e: None,
        )
        # 300 m miss with 50 m lethal radius -> Pk ~ 0 -> mid roll fails to kill.
...
class TestDefaultOnHit:
    ...
    def test_event_logging_content(self):
        ...
        with patch(
            "bvr_marl_core.simulator.core.hit_event_helpers.UnitDestroyedEvent"
        ) as mock_event_class:
...
class TestMissileCCDManagerPostUpdateCCD:
    ...
    def test_first_time_within_lock_gate_logs_engagement(self, mock_distance):
        ...
        with patch.object(manager.hit_calc, "check_and_maybe_hit") as mock_hit_check:
            with patch(
                "bvr_marl_core.simulator.core.hit_event_helpers.MissileEngagementEvent"
            ) as mock_event:
