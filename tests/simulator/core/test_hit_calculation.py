...
from simulator.core.hit_calculation import (
    HitParams,
...
class MockUnit:
    ...
    def __init__(self, unit_id: int, lat: float, lon: float, alt: float, is_destroyed: bool = False):
        self.id = unit_id
        self.position = Mock()
...
class TestMissileHitCalculatorHitDetection:
    ...
    @patch('simulator.core.hit_calculation.MissileHitCalculator._compute_motion_vectors')
...
class TestMissileHitCalculatorPrivateMethods:
    ...
    def test_compute_motion_vectors_with_t0_data(self):
        ...
        with patch('simulator.core.hit_calculation._latlonalt_to_local_m') as mock_convert:
...
class TestMissileHitCalculatorIntegration:
    ...
    def test_multiple_targets_first_hit(self):
        ...
        hit, victim, t_frac = calc.check_and_maybe_hit(
            missile, [target1, target2, target3], 1.0
        )
