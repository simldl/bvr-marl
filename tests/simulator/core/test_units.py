...
from simulator.core.units import Unit, FlyingUnit, Velocity
...
class TestFlyingUnit:
    ...
    def test_flying_unit_initialization_with_attitudes(self):
        ...
        pos = Position(lat=45.0, lon=-123.0, alt=1000.0)
        unit = FlyingUnit(
            name="Bomber", position=pos, yaw_deg=180.0, speed=150.0,
            pitch_deg=5.0, roll_deg=-10.0
        )
    ...
    def test_flying_unit_post_init_calls_super(self):
        ...
        with patch.object(Unit, '__post_init__') as mock_super_post_init:
            unit = FlyingUnit(name="Test", position=pos, yaw_deg=450.0, speed=200.0)
    ...
    @patch('simulator.core.units.geodetic_direct')
    ...
    def test_update_negative_pitch(self):
        ...
        with patch('simulator.core.units.geodetic_direct') as mock_geodetic_direct:
            mock_geodetic_direct.return_value = (45.1, -122.9, 900.0)
            
            pos = Position(lat=45.0, lon=-123.0, alt=1000.0)
            unit = FlyingUnit(name="Diving", position=pos, yaw_deg=0.0, speed=100.0, pitch_deg=-10.0)
            sim = Mock()
    ...
    def test_velocity_property_with_pitch(self):
        ...
        assert vel.vy == pytest.approx(expected_horizontal, abs=1e-10)       # North component (horizontal velocity north)
        assert vel.vz == pytest.approx(expected_vertical, abs=1e-10)         # Up component (vertical velocity)
    ...
    def test_velocity_property_negative_pitch(self):
        ...
        vy_expected = horizontal * math.sin(math.radians(yaw_math))  # North component (negative for south)
...
class TestFlyingUnitIntegration:
    ...
    def test_realistic_flight_scenario(self, mock_geodetic_direct):
        ...
        pos = Position(lat=45.0, lon=-123.0, alt=1000.0)
        aircraft = FlyingUnit(name="Airliner", position=pos, yaw_deg=45.0, speed=250.0, pitch_deg=3.0)
        sim = Mock()
    ...
    def test_different_update_intervals(self):
        ...
        with patch('simulator.core.units.geodetic_direct') as mock_geodetic_direct:
            ...
            pos = Position(lat=45.0, lon=-123.0, alt=1000.0)
            unit = FlyingUnit(name="Variable", position=pos, yaw_deg=0.0, speed=100.0, pitch_deg=5.0)
            sim = Mock()
    ...
    def test_extreme_attitudes(self):
        ...
        with patch('simulator.core.units.geodetic_direct') as mock_geodetic_direct:
            ...
            # Test extreme climb
            unit1 = FlyingUnit(name="Rocket", position=pos.copy(), yaw_deg=0.0, speed=100.0, pitch_deg=89.0)
            unit1.update(1.0, Mock())
            
            # Test extreme dive
            unit2 = FlyingUnit(name="Dive", position=pos.copy(), yaw_deg=0.0, speed=100.0, pitch_deg=-89.0)
...
class TestFlyingUnitEdgeCases:
    ...
    def test_extreme_pitch_angles(self):
        ...
        unit2 = FlyingUnit(name="InvertedDive", position=pos, yaw_deg=0.0, speed=100.0, pitch_deg=-120.0)
        vel2 = unit2.velocity
    ...
    def test_multiple_consecutive_updates(self):
        ...
        with patch('simulator.core.units.geodetic_direct', side_effect=side_effect):
            pos = Position(lat=45.0, lon=-123.0, alt=1000.0)
