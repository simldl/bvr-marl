...
def test_detailed_missile_tracking_and_guidance(sim, map_limits):
    ...
    from bvr_marl_core.radar.core.utils import _angles_dist, enu_to_geodetic, geodetic_to_enu
    ...
    # Consistency check for ENU <-> Geodetic round-trip
    ...
    for t in range(170):
        ...
        if not (target.id in sim.active_units and missile.id in sim.active_units):
            ...
            if missile.id not in sim.active_units:
                ...
                if min_dist_m <= 500.0:
                    print(
                        f"[RESULT] HIT (missile removed within hit radius, min_dist={min_dist_m:.1f} m)"
                    )
                    hit = True
            break
