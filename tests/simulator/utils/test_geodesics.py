...
from air_to_air_rl.simulator.utils.geodesics import (
...
class TestGeodeticDirect:
    ...
    def test_east_movement(self):
        ...
        lat, lon, alt = geodetic_direct(
            45.0, -123.0, 1000.0, 90.0, 78847.0, 0.0
        )  # ~1 degree at 45°N
        assert lat == pytest.approx(
            45.0, abs=1e-2
        )  # Geodetic calculations may have small latitude changes
        assert lon > -123.0
        assert alt == pytest.approx(1000.0, abs=1e-10)
...
class TestGeodeticIntegration:
    ...
    def test_distance_bearing_consistency(self):
        ...
        geodetic_distance_km(lat1, lon1, alt1, lat2, lon2, alt2)
        ...
        calc_lat, calc_lon, _ = geodetic_direct(
            lat1, lon1, alt1, bearing, horizontal_dist * 1000, 0
        )
    ...
    def test_round_trip_calculation(self):
        ...
        # Go to new position
        new_lat, new_lon, _ = geodetic_direct(
            original_lat, original_lon, 1000.0, bearing, distance_m, 0
        )

        # Calculate bearing and distance back
        back_bearing = geodetic_bearing_deg(new_lat, new_lon, original_lat, original_lon)
        back_distance_km = geodetic_distance_km(
            new_lat, new_lon, 1000.0, original_lat, original_lon, 1000.0
        )
    ...
    def test_multiple_short_segments_vs_direct(self):
        ...
        # Direct path
        direct_lat, direct_lon, _ = geodetic_direct(
            start_lat, start_lon, 0, bearing, total_distance_m, 0
        )

        # Segmented path
        current_lat, current_lon = start_lat, start_lon
        for _ in range(num_segments):
            current_lat, current_lon, _ = geodetic_direct(
                current_lat, current_lon, 0, bearing, segment_distance_m, 0
            )
...
class TestGeodeticEdgeCases:
    ...
    def test_very_large_distances(self):
        ...
        lat, lon, alt = geodetic_direct(
            0.0, 0.0, 0.0, 0.0, 10007543.0, 0.0
        )  # ~1/4 Earth circumference
        assert abs(lat) <= 90
        assert abs(lon) <= 180
