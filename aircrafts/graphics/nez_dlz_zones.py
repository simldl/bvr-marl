...
def create_nez_dlz_plot():
    ...
    # ==================== Get actual missile parameters from codebase ====================
    ...
    amraam_seeker_sensitivity = 1.2
    amraam_life_time_s = 120.0
    amraam_mass_kg = 160.0

    meteor_base_range_km = 200.0
    meteor_min_range_km = 2.0
    meteor_seeker_sensitivity = 1.3
    meteor_life_time_s = 160.0
    meteor_mass_kg = 190.0

    # Try to load directly from missile Python classes by reading source code
    try:
        import inspect
        from missiles.fox3.amraam import AIM120_AMRAAM
        from missiles.fox3.meteor import Meteor
        ...
        print(f"[WARNING] Using hardcoded default values")

    # ==================== DLZ zone calculations ====================
    ...
    amraam_r_tr, amraam_r_pi, amraam_r_aero, amraam_r_max = calculate_dlz_zones(amraam_min_range_km, amraam_base_range_km)
    meteor_r_tr, meteor_r_pi, meteor_r_aero, meteor_r_max = calculate_dlz_zones(meteor_min_range_km, meteor_base_range_km)

    # ==================== Plot 1: DLZ Zone Comparison ====================
    ax = axes[0]

    missiles = [
        ('AIM-120D AMRAAM', amraam_min_range_km, amraam_r_tr, amraam_r_pi, amraam_r_max,
         ['#95E1D3', '#6DDF9B', '#38E19B', '#0BC965'], 0.7),
        ('Meteor', meteor_min_range_km, meteor_r_tr, meteor_r_pi, meteor_r_max,
         ['#FFE66D', '#FFC700', '#FF9800', '#FF6B35'], 0.7),
    ]
    ...
    for missile_name, r_min, r_tr, r_pi, r_max, colors, alpha_base in missiles:
        ...
        # R1: Too close
        ax.barh(y_base, r_min, left=0, height=y_height, color=colors[0], alpha=alpha_base, edgecolor='black', linewidth=2)

        # R2: NEZ
        ax.barh(y_base, r_tr - r_min, left=r_min, height=y_height, color=colors[1], alpha=alpha_base, edgecolor='black', linewidth=2)

        # R3: Optimal
        ax.barh(y_base, r_pi - r_tr, left=r_tr, height=y_height, color=colors[2], alpha=alpha_base, edgecolor='black', linewidth=2)

        # R4: Extended
        ax.barh(y_base, r_max - r_pi, left=r_pi, height=y_height, color=colors[3], alpha=alpha_base, edgecolor='black', linewidth=2)

        # Add missile name above the bar
        ax.text(0, y_base + y_height + 0.15, missile_name, ha='left', va='bottom', fontsize=11, fontweight='bold')

        # Add labels on bars
        ax.text(r_min / 2, y_base + y_height / 2, 'R1', ha='center', va='center', fontsize=9, fontweight='bold', color='black')
        ax.text((r_min + r_tr) / 2, y_base + y_height / 2, 'R2\nNEZ', ha='center', va='center', fontsize=8, fontweight='bold', color='black')
        ax.text((r_tr + r_pi) / 2, y_base + y_height / 2, 'R3\nOptimal', ha='center', va='center', fontsize=8, fontweight='bold', color='black')
        ax.text((r_pi + r_max) / 2, y_base + y_height / 2, 'R4\nExtended', ha='center', va='center', fontsize=8, fontweight='bold', color='black')

        # Add max range label
        ax.text(r_max + 1.5, y_base + y_height / 2, f'{r_max:.1f} km', ha='left', va='center', fontsize=10, fontweight='bold')
    ...
    # ==================== Plot 2: Fox-3 AMRAAM NEZ vs Aspect Angle ====================
    ax = axes[1]
    ...
    # Plot ranges
    ax.fill_between(aspect_angles, 0, nez_range_fox3, alpha=0.3, color='green', label='R2: NEZ (Best)')
    ax.fill_between(aspect_angles, nez_range_fox3, r3_range_fox3, alpha=0.2, color='yellow', label='R3: Optimal')
    ax.fill_between(aspect_angles, r3_range_fox3, r_max_fox3, alpha=0.1, color='orange', label='R4: Extended')

    ax.plot(aspect_angles, nez_range_fox3, 'g-', linewidth=2.5, label='NEZ Boundary')
    ax.plot(aspect_angles, r3_range_fox3, 'darkorange', linewidth=2.5, linestyle='--', label='R3 Boundary')
    ax.plot(aspect_angles, r_max_fox3, color='darkred', linewidth=2.5, linestyle=':', label='Max Range')

    # Highlight aspect angles
    ax.axvline(0, color='blue', linestyle=':', alpha=0.5, linewidth=1.5)
    ax.text(0, amraam_r_max * 0.85, '0°\nHead-on\n(Low RCS)', ha='center', fontsize=8, color='blue', fontweight='bold')

    ax.axvline(90, color='darkgreen', linestyle=':', alpha=0.5, linewidth=2)
    ax.text(90, amraam_r_max * 0.85, '90°\nBeam\n(High RCS)', ha='center', fontsize=8, color='darkgreen', fontweight='bold')

    ax.axvline(180, color='blue', linestyle=':', alpha=0.5, linewidth=1.5)
    ax.text(180, amraam_r_max * 0.85, '180°\nTail\n(Low RCS)', ha='center', fontsize=8, color='blue', fontweight='bold')
    ...
    ax.set_title(f'Fox-3 AMRAAM NEZ vs Target Aspect ({amraam_base_range_km:.0f}km base, RCS dependent)', fontsize=12, fontweight='bold')
    ax.set_xlim(0, 180)
