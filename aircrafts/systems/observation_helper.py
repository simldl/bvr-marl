...
class ObservationHelper:
    ...
    def get_geometry_kinematics(self, target) -> Dict[str, Any]:
        ...
        try:
            ...
            # Aspect angle (target's perspective)
            ...
            # Angle-off-tail (180 - aspect)
            aot = 180.0 - aspect

            # --- Closure rate (project both velocities onto LOS) ---
            own_speed = float(self.aircraft.speed)
            tgt_speed = float(getattr(target, 'speed', 250.0))

            # LOS unit is from own → target; bearing is CW-from-North
            u_bearing = np.radians(bearing)
            # Own projection onto LOS uses ATA: own_speed * cos(ATA)
            own_los = own_speed * np.cos(np.radians(ata))

            # Target projection: need target heading vs. (target → own) bearing
            bearing_from_target = geodetic_bearing_deg(
                target.position.lat, target.position.lon,
                self.aircraft.position.lat, self.aircraft.position.lon
            )
            ...
            # V_tgt ⋅ u_LOS = - tgt_speed * cos(angle to own), so subtracting a negative adds
            tgt_los = - tgt_speed * np.cos(np.radians(tgt_to_own_err))

            closure = own_los - tgt_los  # = (own⋅u) − (tgt⋅u)
    ...
    def get_track_target_quality(self, selected_target=None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extract top-N tracks with scores
            top_n = min(5, num_tracks)
            top_tracks = [
                {'state': state.tolist(), 'score': score}
                for state, score in prioritized[:top_n]
            ]

            # Find selected target's score
            selected_score = 0.0
            if selected_target:
                # Match by nearest track position (state[:3]) to selected target's relative ENU
                # (Assumes track states are in own-ENU; otherwise this degrades gracefully.)
                try:
                    # Build own→target ENU (flat-earth small-angle)
                    dlat = (selected_target.position.lat - self.aircraft.position.lat) * 111000.0
                    dlon = (selected_target.position.lon - self.aircraft.position.lon) * 111000.0 * \
    ...
    def get_fire_feasibility(self, target) -> Dict[str, Any]:
        ...
        try:
            ...
            # Compute gimbal_ok from geometry
            geom = self.get_geometry_kinematics(target)
            gimbal_ok = geom['ata_deg'] < (self.aircraft.radar.h_fov_deg / 2.0) if geom['valid'] else False

            # Check radar range
            radar_range_ok = geom['slant_range_m'] <= (self.aircraft.radar.max_range_m * 0.95) if geom['valid'] else False
    ...
    def get_threat_warnings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # FUTURE: Track first warning time for time_since_first
    ...
    def get_weapons_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # FUTURE: Implement missile inventory by class (Fox-1/2/3)
            remaining_by_class = {
                'fox1': 0,
                'fox2': 0,
                'fox3': remaining  # Simplification: assume all are Fox-3
            }
            ...
            if hasattr(self.aircraft, 'weapons') and hasattr(self.aircraft.weapons, 'gun'):
                gun = self.aircraft.weapons.gun
                gun_ammo = getattr(gun, 'current_ammo', 0)
                # FUTURE: Expose gun cooldown from GunSystem

            # Missile cooldown (from ActionProcessor state if available)
