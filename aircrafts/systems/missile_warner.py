...
class MissileWarner:
    ...
    def check_for_new_missiles(self, sim_time: float, sim):
        ...
        # Track which missile IDs are still active in the simulation
        active_missile_ids = set()

        for unit in getattr(sim, "active_units", {}).values():
            if getattr(unit, "is_missile", False) \
               and getattr(unit, "target", None) is self.parent:
                active_missile_ids.add(unit.id)

                # Add new missile warnings
                if unit.id not in self._active_warnings \
                   and not self._is_pending(unit.id):
                    delay = float(max(np.random.normal(self.detection_delay_s, self.detection_delay_std), 0.05))
                    warn_time = float(sim_time + delay)
                    self._pending_warnings.append((warn_time, unit.id, unit))

        # Clean up warnings for missiles that no longer exist
        self._active_warnings = self._active_warnings.intersection(active_missile_ids)

        # Clean up pending warnings for missiles that no longer exist
        self._pending_warnings = [
            (wt, mid, m) for (wt, mid, m) in self._pending_warnings
    ...
    def update(self, sim_time: float):
        ...
        self._pending_warnings = [(wt, mid, m) for (wt, mid, m) in self._pending_warnings if sim_time < wt]
        for missile in due:
