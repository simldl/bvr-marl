...
class PassiveRadar:
    def __init__(self, angular_error_deg: float = 5.0, range_error_m: float = 2000.0, max_age_s: float = 3.0):
    ...
    def receive_emission(self, emitter: Unit, receiver: Unit, timestamp: float):
        ...
        dlon_deg = float(getattr(emitter.position, "lon", 0.0) - getattr(receiver.position, "lon", 0.0))
        ...
        rel_angle = (azimuth_deg - float(getattr(receiver, "yaw_deg", 0.0)) + err + 540.0) % 360.0 - 180.0
    ...
    def clear_old(self, current_time: float):
        self.detections = [d for d in self.detections if (current_time - d["timestamp"]) <= self.max_age_s]