...
class AircraftControlSystem:
    ...
    def _check_boundary_violation(self, position):
        ...
        m = self.parent
        lat_violated = (position.lat < m.map_limits.bottom_lat or
                       position.lat > m.map_limits.top_lat)
        lon_violated = (position.lon < m.map_limits.left_lon or
                       position.lon > m.map_limits.right_lon)
        return lat_violated or lon_violated

    def _clamp_position_to_boundary(self, position):
        pos = position
        m = self.parent
        # Only clamp altitude, not lat/lon (those trigger removal)
    ...
    def update_movement(self, tick_secs: float):
        lat, lon, alt, spd, new_yaw, new_pitch, new_roll = self.parent.physics.compute_movement(
            self.parent.position, self.yaw_deg, self.desired_yaw_deg,
            self.pitch_deg, self.desired_pitch_deg, self.parent.speed,
            self.throttle, tick_secs
        )
        ...
        # Update aircraft attitude to match control system
