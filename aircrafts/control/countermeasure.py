import logging
from aircrafts.control.countermeasure_objects import create_countermeasure
...
class AircraftCountermeasureSystem:
    ...
    def __init__(self, parent):
        ...
        # Track active countermeasures in the simulation
        self.active_countermeasures = []

        # Deployment parameters (can be customized per aircraft type)
        self.flare_params = {
            'lifetime_s': 5.0,
            'effectiveness': 1.0,
            'decay_rate': 0.3,
    ...
    def launch_chaff(self, simulator=None):
        ...
        if self.chaff > 0:
            ...
            # Create physical chaff cloud object
    ...
    def activate_ecm(self):
        """
        Activate ECM (electronic countermeasures).

        ECM works differently - it's an active system on the aircraft,
        not a physical object in the simulation.
        """
        if self.ecm > 0:
            # ECM is a state on the aircraft, not a physical object
    ...
    def cleanup_expired_countermeasures(self):
        ...
        self.active_countermeasures = [
            cm for cm in self.active_countermeasures
            if not getattr(cm, 'should_be_removed', False)
        ]
    ...
    def get_nearby_countermeasures(self, position, max_range_m=1000.0):
        ...
        for cm in self.active_countermeasures:
            if hasattr(cm, 'position'):
                distance_m = geodetic_distance_km(
                    position.lat, position.lon, position.alt,
                    cm.position.lat, cm.position.lon, cm.position.alt
                ) * 1000.0
