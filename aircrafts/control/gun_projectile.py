...
class GunProjectilePhysics:
    ...
    def __init__(self, initial_velocity_mps: float = 1000.0, drag_coefficient: float = 0.1, mass_kg: float = 0.1):
    ...
    def update_position(self, projectile, dt_s: float):
        ...
        if speed > 0:
            drag_force = -0.5 * self.drag_coefficient * speed**2 * (velocity_vector / speed)
            acceleration_drag = drag_force / self.mass_kg
        else:
            acceleration_drag = np.zeros(3)
        
        # Gravity (downward)
        acceleration_gravity = np.array([0, 0, -self.gravity_mps2])
        
        # Total acceleration
        total_acceleration = acceleration_drag + acceleration_gravity
        
        # Update velocity
        velocity_vector += total_acceleration * dt_s
        from simulator.core.units import Velocity
        projectile.velocity = Velocity(velocity_vector[0], velocity_vector[1], velocity_vector[2])
        
        # Update position
        projectile.position.lat += velocity_vector[0] * dt_s / 111000.0  # Rough conversion
        projectile.position.lon += velocity_vector[1] * dt_s / (111000.0 * np.cos(np.radians(projectile.position.lat)))
...
class GunProjectile(FlyingUnit):
    ...
    def __init__(self, firing_time: datetime, source_aircraft, target_position, source_velocity, 
                muzzle_velocity_mps: float = 1000.0, max_range_m: float = 2000.0,
                group: str = None, kill_radius_m: float = 5.0):
        ...
        # Use source group by default (avoids 'neutral' FF corner cases)
        ...
        # Aircraft velocity (m/s, ENU-aligned)
        aircraft_velocity = np.array([source_velocity.vx, source_velocity.vy, source_velocity.vz], dtype=float)

        # Build ENU delta (meters) from lon/lat/alt target tuple (lon, lat, alt)
        dE = (target_position[0] - self.position.lon) * 111000.0 * np.cos(np.radians(self.position.lat))
        ...
        # Normalize direction; if zero, shoot forward horizontally
        ...
        muzzle_velocity_vector = target_dir_m * float(muzzle_velocity_mps)
        initial_velocity = aircraft_velocity + muzzle_velocity_vector

        from simulator.core.units import Velocity
        self.velocity = Velocity(initial_velocity[0], initial_velocity[1], initial_velocity[2])

        # Physics
        self.physics = GunProjectilePhysics(
            initial_velocity_mps=muzzle_velocity_mps,
    ...
    def update(self, tick_secs: float, sim: Simulator) -> List[Event]:
        ...
        # Track distance traveled (meters)
        dx = (new_position[0] - old_position[0]) * 111000.0 * np.cos(np.radians((new_position[1] + old_position[1]) * 0.5))
        dy = (new_position[1] - old_position[1]) * 111000.0
        dz = (new_position[2] - old_position[2])
        distance_step = float(np.linalg.norm([dx, dy, dz]))
        ...
        # Check for hits against enemy units
        for unit_id, unit in sim.active_units.items():
            if (unit.id != self.id and 
                unit.group != self.group and 
                unit.type == "Aircraft" and
                unit.id != self.source_aircraft.id):
                
                # Calculate distance to potential target
                distance_m = self._calculate_distance_to_unit(unit)
                
                if distance_m <= self.kill_radius_m:
                    # Hit detected
                    from simulator.core.events import UnitDestroyedEvent
                    events.append(UnitDestroyedEvent(
                        origin=sim,
                        unit_killer=self.source_aircraft,
                        unit_destroyed=unit
                    ))
                    sim.remove_unit(self)
                    break
    ...
    def _calculate_distance_to_unit(self, unit) -> float:
        """Calculate distance to another unit in meters."""
        # Simple 3D distance calculation
        dx = (unit.position.lon - self.position.lon) * 111000 * np.cos(np.radians(self.position.lat))
...
class GunSystem:
    ...
    def __init__(self, parent_aircraft, max_ammo: int = 500, 
                 muzzle_velocity_mps: float = 1000.0, max_range_m: float = 2000.0,
                 burst_size: int = 10, burst_duration_s: float = 0.1):
        ...
        self.min_fire_interval_s = 0.05  # Minimum time between trigger pulls
    ...
    def _fire_burst(self, sim: Simulator, target_position: tuple = None) -> List[GunProjectile]:
        ...
        # Default target position (straight ahead)
        if target_position is None:
            range_ahead = 1000.0  # 1km ahead
            target_position = (
                self.parent.position.lon + range_ahead * np.cos(np.radians(self.parent.yaw_deg)) / 111000,
                self.parent.position.lat + range_ahead * np.sin(np.radians(self.parent.yaw_deg)) / 111000,
        ...
        for i in range(rounds_to_fire):
            ...
            spread_target = (
                target_position[0] + spread_x / (111000.0 * np.cos(np.radians(self.parent.position.lat))),
            ...
            projectile = GunProjectile(
                ...
                kill_radius_m=getattr(sim, 'gun_hit_radius_m', 5.0)  # Use simulator's configured value
            )
